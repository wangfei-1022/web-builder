    ...
    <el-dialog :visible.sync="batchImportVisible" :close-on-click-modal="false" width="500px" :title='importTitle'>
      <el-form class="contact-form" ref="batchImportRef" :model="importForm" :rules="importFormRule" label-width="100px" size="mini">
        <div style="text-align: center;margin-bottom: 20px;">
          <el-button size="mini" type="primary" icon="el-icon-download" @click="downloadTemplateFn">{{$t("uexpressProduct.product_area_import_template")}}</el-button>
        </div>
...
export default {
	...
	methods: {
		...
		addFn() {
      this.$router.push({ name: "ProductAreaAdd"});
		},
    edit(item) {
			this.$router.push({
