				...
				<div class="multiple-form-wrap">
					<el-form ref="formData" :inline="true" :model="formData" label-width="100px">
						...
						<el-form-item :label='$t("uexpressProduct.supplier")' prop="supplierId">
							<type-select v-model="formData.supplierId" type="supplier" @selected="list" :requestParams="{partnerRole: 17011}"
              ></type-select>
						</el-form-item>
    ...
    <el-dialog :visible.sync="batchImportVisible" :close-on-click-modal="false" width="500px" :title='importTitle'>
      <el-form class="contact-form" ref="batchImportRef" :model="importForm" :rules="importFormRule" label-width="100px" size="mini">
        <div style="text-align: center;margin-bottom: 20px;">
          <el-button size="mini" type="primary" icon="el-icon-download" @click="downloadTemplateFn">{{$t("uexpressProduct.channel_area_import_template")}}</el-button>
        </div>
...
<script>
import { areaChannelListApi, areaChannelActivateApi, areaChannelForbiddenApi, importChannelAreaListApi, importChannelAreaDetailApi } from "@/api/uexpress/channelArea";
...
export default {
    ...
    addFn() {
      this.$router.push({ name: "ChannelAreaAdd"});
		},
    edit(item) {
			this.$router.push({
