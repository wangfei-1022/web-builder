  ...
  <div class="right-main-wrapper" v-loading="loading">
    ...
    <div class="mt-20">
      <div class="top-btn-wrap mb-10 mt-10">
        ...
        <el-button type="primary" size="mini" @click="supplierExchangeFn">{{ $t("uexpressOrder.supplierExchange") }}</el-button>
    ...
    <el-dialog :visible.sync="exchangeVisible" :close-on-click-modal="false" width="500px" :title='$t("uexpressOrder.supplierExchange")'>
      <el-form class="contact-form" ref="returnRef" :model="exchangeForm" :rules="exchangeFormRule" label-width="100px" size="mini">
        <el-form-item :label='$t("uexpressOrder.boxNo")' prop="boxNo">
          <el-input v-model="exchangeForm.boxNo" />
        </el-form-item>
      </el-form>
      <div slot="footer" class="option-btn-wrap">
        <el-button size="mini" @click="exchangeVisible = false">{{ $t("common.cancel") }}</el-button>
        <el-button size="mini" type="primary" @click="supplierExchangeConfirmFn" :loading="exchangeLoading">{{ $t("common.confirm") }}</el-button>
      </div>
    </el-dialog>

    <el-dialog :visible.sync="batchImportVisible" :close-on-click-modal="false" width="500px" :title='$t("uexpressOrder.exportDeclareImport")'>
      <el-form class="contact-form" ref="batchImportRef" :model="importForm" :rules="importFormRule" label-width="100px" size="mini">
        <div style="text-align: center;margin-bottom: 20px;">
          <el-button size="mini" type="primary" icon="el-icon-download" @click="downloadTemplateFn">{{$t("uexpressOrder.exportDeclareImportTemplate")}}</el-button>
        </div>
...
import {
  ...
  exportDeclareImportApi, // 报关查验导入
  exportDeclareSupplierDestructionApi, // 报关查验-供应商销毁
  exportDeclareReturnWarehouseApi, // 报关查验-退回我司仓库
  exportDeclareSupplierExchangeApi // 报关查验-供应商换货出库
...
export default {
  ...
  data() {
    return {
      ...
      exchangeFormRule: {
        boxNo: [{required: true, message: this.$t("common.cannot_be_null"),	trigger: "change"}]
      },
      ...
      columnFields: [{
        ...
        {
          field: "boxNo",
          headerName: this.$t("uexpressOrder.boxNo"),
          minWidth: 120
        },
        {
          field: "outboundNo",
          headerName: this.$t("uexpressOrder.outboundNo"),
          minWidth: 120
        },
        {
  ...
  methods: {
    // 导入
    downloadTemplateFn() {
      getTemplateFileApi(12).then(res => {
    ...
    batchImportConfirmFn() {
      this.$refs.batchImportRef.validate(valid => {
        if (valid) {
          ...
          exportDeclareImportApi(data).then(res => {
            if (res.ok) {
    ...
    supplierDestructionFn() {
      ...
      this.$confirm(this.$t("uexpressOrder.supplierDestructionMention", { num: ids.length }), this.$t("common.mention"), {
        ...
        exportDeclareSupplierDestructionApi(data).then(res => {
          if (res.ok) {
            this.getList()
    ...
    returnWarehouseFn() {
      ...
      this.$confirm(this.$t("uexpressOrder.returnWarehouseMention", { num: ids.length }), this.$t("common.mention"), {
        ...
        exportDeclareReturnWarehouseApi(data).then(res => {
          if (res.ok) {
            this.getList()
            this.$message.success(this.$t("uexpressOrder.operSuccess"));
          }
    ...
    supplierExchangeFn() {
      ...
      for (let index = 0, len = arr.length; index < len; index++) {
        ...
        if (item.abnormalityState !== 1) {
          // 【异常状态】=【待处理】；
          // 只有【待处理】的订单，才能选择【供应商换箱出货】的处理方案
          this.$message.error(this.$t('uexpressOrder.supplierExchangeErrorInfo'))
          return
        }
      }
      this.exchangeForm = {
    ...
    supplierExchangeConfirmFn() {
      this.$refs.returnRef.validate(valid => {
        if (valid) {
          ...
          exportDeclareSupplierExchangeApi(data).then(res => {
            if (res.ok) {
              this.getList()
