...
to the Carrier or his agents for failure to notify)"  v-model="customerBlForm.notifyAddress" maxlength="179"
          ...
          , { validator: (rule, value, callback)=>validateCharacterWrap(rule, value, callback,15,200),message:`${$t('sea.atMost5TextsCanBeAddedA')} 15 X 200  ${$t('sea.atMost5TextsCanBeAddedB')}`, trigger: 'change' }]">
...
<!--                <el-checkbox v-model="customerBlForm.disPlayFlag" style="margin-right:20%">{{ $t('sea.showOnBillOfLading') }}</el-checkbox>-->
<!--              </div>-->
            </div>
            <!--            原产地-->
...
<!--            第二通知方信息-->
            <el-form-item prop="secondNotifyAddress" :rules="[ { required: customerBlForm.blSignType !==1,message:$t('sea.pleaseEnter'),trigger: 'change'},
             ...
             'For delivery of goods please apply to' " style="width: 100%;" >
              <el-input type="textarea"  v-input-auto="'upperCase'" v-model="customerBlForm.secondNotifyAddress"
                        @input="descInput('secondNotifyAddress',35,5)"  maxlength="179" show-word-limit rows="5"></el-input>
            ...
            <el-form-item prop="otherNotifyInfo"  :label="$t('sea.otherNotifyingPartyInformation')" style="width: 100%;"
              ...
              <el-input type="textarea" v-input-auto="'upperCase'" v-model="customerBlForm.otherNotifyInfo" @input="descInput('otherNotifyInfo',35,5)"
                        maxlength="179" rows="5" ></el-input>
            </el-form-item>
            <!--            在提单上显示货代信息-->
            <div>
            ...
            <el-form-item prop="forwarderInfo"  label="" style="width: 100%;" :rules="[{ validator: (rule, value, callback)=>validateCharacterWrap(rule, value, callback,35,4), trigger: 'change' } ]">
              <el-input type="textarea" v-input-auto="'upperCase'" v-model="customerBlForm.forwarderInfo"  @input="descInput('forwarderInfo',35,4)"
                        maxlength="143" show-word-limit rows="5" ></el-input>
            </el-form-item>
    ...
    <div v-if="Number(orderDetailsData.carrierId) === 5" class="btn_box" :style="{width: `calc(100% - ${withoutAnimation ? 180 :54}px)`}">
      ...
      <el-button v-if="!blId" :disabled="isEdit" type="primary" @click="sendReplenishment" size="mini">{{ $t('sea.sendReplenishment') }}</el-button>
      <el-button v-if="[30,40,50,80].indexOf(orderDetailsData.orderState)>-1 || !blId" :disabled="isEdit" type="primary" @click="save" size="mini">{{ $t('sea.save') }}</el-button>
      <el-button v-if="!blId" :disabled="isEdit" type="primary" @click="replenishmentCompleted" size="mini">{{ $t('sea.replenishmentCompleted') }}</el-button>
...
import {
  customerSaveAndSendSupplementaryApi, customerSupplementaryCompletedApi,
...
export default {
  ...
  created() {
    ...
    if(!this.customerBlForm.consigneeIndianInfo){
      this.$set(this.customerBlForm, 'consigneeIndianInfo', {})
    }
    if(!this.customerBlForm.notifyPartyIndianInfo){
      this.$set(this.customerBlForm, 'notifyPartyIndianInfo', {})
    }
    // if(!this.blId && !this.customerBlForm.rateType){
    ...
    // }
    if(!this.blId && !this.customerBlForm.rateReferenceNo){
      this.$set(this.customerBlForm,'rateReferenceNo', this.customerBlForm.serviceContractNo)
    }
    ...
    if (this.$isNotEmpty(this.customerBlForm.soNoSet)) {
      this.$set(this.customerBlForm, 'bookingNos', this.customerBlForm.soNoSet)
...
SHENZHEN`)
    ...
    async save() {
      ...
      if (!this.blId || this.blId && !this.customerBlForm.carrierBlId) {
        // 先提交
        if (this.customerBlForm.blSignType === 1) {
          this.$refs.customerBlForm.validate(async valid => {
            if (valid) {
              await this.saveApi()
          ...
          if (this.customerBlForm.blSignType === 1) { // MBL
            this.$refs.customerBlForm.validate(async (valid, obj) => {
              if (valid) { // 如果必填项均有值则提示 changeShippingBl
                ...
                this.mixinsFormScrollIntoView()
                this.$message.warning(this.$t('sea.hasShippingBl'))
              }
            })
    ...
    async saveApi() {
      let assemblyData = await this.assemblyData()
      this.loading = true
      let res
      const data = lodash.cloneDeep(assemblyData)
      if (this.blId) {
        data.blId = this.blId
        res = await seaBlEditSaveApi(data)
      } else {
        res = await seaBlSaveApi(data)
      ...
      this.loading = false
      if (res.ok) {
        // if (this.blId && this.customerBlForm.carrierBlId) {
        //   this.$emit('returnBillDetails')
        // }
    ...
    async sendReplenishmentCallBack(val){
      ...
      this.loading = true
      let res = await customerSaveAndSendSupplementaryApi(data)
      this.loading = false
      if(res.ok){
        this.$message.success(this.$t('sea.submittedSuccessfully'))
        this.$emit('returnBillDetails')
    ...
    async replenishmentAccountNoCallBack(val){
      ...
      this.loading = true
      let res = await customerSupplementaryCompletedApi(data)
      this.loading = false
      if(res.ok){
        this.$message.success(this.$t('sea.submittedSuccessfully'))
        this.$emit('returnBillDetails')
