...
export default {
	...
	computed: {
		...
		// 判断业务类型
		checkEntryBusinessType() {
			const compareObj = this.pools[0] || false
			this.businessType = this.pools[0].businessType
			if (compareObj) {
				return this.pools.some(item => item.businessType !== compareObj.businessType)
			}
			return false
		},
	...
	methods: {
		...
		async nextStep() {
			...
			if (this.checkEntryBusinessType) {
				this.$message({
					type: 'warning',
					message: ' 所选费用的业务类型不一致，请选择业务类型一致的费用入账'
				})
				return false
			}
