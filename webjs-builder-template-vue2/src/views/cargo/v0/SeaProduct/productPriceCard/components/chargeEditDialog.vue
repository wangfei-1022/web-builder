<template>
  <div>
    <el-dialog :visible.sync="visible" :title="$t('warehousePriceCard.edit')" width="600px" :close-on-click-modal="false"
      @close="closeFn">
        ...
        <div v-if="!IS_BETWEEN">
          <div class="module">
            ...
            <div class="content-item">
              ...
              <el-input size="mini" v-model="conditionList[0].startValue"
                oninput="value=value.replace(/^\D*(\d*(?:\.\d{0,3})?).*$/g,'$1')" :disabled="isDetail"></el-input>
          ...
          <div class="module">
            ...
            <div class="content-item">
              ...
              <el-input size="mini" v-model="conditionList[0].costPrice"
                oninput="value=value.replace(/^\D*(\d*(?:\.\d{0,3})?).*$/g,'$1')" :disabled="isDetail"></el-input>
            </div>
          </div>
        </div>
          ...
          <div class="module">
            ...
            <div class="content-item" v-for="(item, index) in conditionList" :key="index">
              ...
              <span v-if="item.conditionEndSignTypeName">
                ...
                <el-input size="mini" v-model="item.endValue" @change="endChange(index)"
                  oninput="value=value.replace(/^\D*(\d*(?:\.\d{0,3})?).*$/g,'$1')" :disabled="isDetail"></el-input>
                <span>{{ item.unitTypeName }}</span>
              </span>
              <span v-if="!item.conditionEndSignTypeName" style="width: 145px;height: 10px;"></span>
              <span>{{ $t('warehousePriceCard.price') }}</span>
              <el-input size="mini" v-model="item.costPrice"
                oninput="value=value.replace(/^\D*(\d*(?:\.\d{0,3})?).*$/g,'$1')" :disabled="isDetail"></el-input>
              <i class="icon iconfont icon-minus-circle" v-if="conditionList.length > 1 && !isDetail"
                @click="removeFn(index)"></i>
              <i class="icon iconfont icon-add-circle" v-if="index == conditionList.length - 1 && !isDetail"
                @click="addFn"></i>
            </div>
          </div>
        </div>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" size="mini" @click="confirmFn" v-if="!isDetail"> {{ $t('warehousePriceCard.confirm') }}
        </el-button>
        <el-button size="mini" @click="closeFn" v-if="!isDetail"> {{ $t('warehousePriceCard.cancel') }} </el-button>
        <el-button type="primary" size="mini" plain @click="closeFn" v-if="isDetail"> {{ $t('warehousePriceCard.close') }}
        </el-button>
      </span>
    </el-dialog>
  </div>
