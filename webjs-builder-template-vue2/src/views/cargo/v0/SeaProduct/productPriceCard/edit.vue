...
<script>
import {
  getSeaProductLineListApi, getPriceCardTemplateApi, editSeatPriceCardApi, getSeatPriceCardDetailApi, checkValiditySeatPriceCardApi, saveSeatPriceCardApi
} from "@/api/seaProduct";
...
export default {
  ...
  methods: {
    ...
    async initDetail () {
      ...
      const result = await getPriceCardTemplateApi({ "carrierCode": this.formModel.carrierCode })
      if (result.ok && res.ok) {
      ...
      this.$refs.formRef.validate(async valid => {
        if (valid) {
          let param = {
            ...
            routeCode: this.formModel.routeCode,
            dataSource: this.formModel.dataSource
          }
          const res = await checkValiditySeatPriceCardApi(param)
          if (res && res.ok) {
            this.addVisible = true
          }
      ...
      this.$refs.formRef.validate(async valid => {
        if (valid) {
          ...
          if (this.$route.query.type === "copy") {
            const res = await saveSeatPriceCardApi(this.formModel)
            if (res && res.ok) {
            ...
            const result = await editSeatPriceCardApi(this.$route.query.id, this.formModel)
            if (result && result.ok) {
              this.$message.success(this.$t('seaProduct.successSaved'))
              this.backList()
            }
