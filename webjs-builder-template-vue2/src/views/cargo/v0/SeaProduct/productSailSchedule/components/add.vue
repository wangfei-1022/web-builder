<template>
  <!-- 新增船期 -->
  <el-dialog :close-on-click-modal="false" :title="$t('seaProduct.shipScheduleAdd')" :visible.sync="visible"
...
<script>
import { saveSeaProductSailScheduleApi, getSeaProductPriceCardManageListApi } from '@/api/seaProduct'

export default {
  name: 'ProductSailScheduleAdd',
  props: {
    visible: {
      type: Boolean,
      default: false
  ...
  methods: {
    ...
    addFn () {
      ...
      this.priceCardList.unshift({ id: "", priceCardNo: "", priceCardName: "", transitNumber: "", transJson: "", transType: "", dataSource: "" })
    },
    addVisibleFn () {
      let priceCardTransList = []
      let flag = false
    ...
    submitBaseInfo () {
      this.$refs.formRef.validate(valid => {
        if (valid) {
          ...
          saveSeaProductSailScheduleApi(this.formModel).then(res => {
            if (res && res.ok) {
              this.$emit('refresh')
