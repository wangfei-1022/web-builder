      ...
      <div class="form-module-item">
        <el-form ref="subscriptionMode" :model="subscriptionMode" :rules="subscriptionRules" label-width="95px"
          ...
          <el-form-item :label="$t('seaProduct.effectiveDate')" prop="validTimeRange">
            ...
            <!-- <CommonPicker v-model="subscriptionMode.expiryDateStart" pikcer-type="datetime"
            ...
            <template slot-scope="{row, $index}">
              <el-form-item :prop="`charges[${$index}].podJsonListName`">
                <el-select v-model="row.podJsonListName" multiple filterable :placeholder="$t('seaProduct.placeholder')"
                  @change="selectedpodCode($event, $index)" clearable size="mini" style="width:100%"
...
<script>
import { seaportlistAllAPi, saveSeaProductSubscriptionApi } from '@/api/seaProduct'
...
export default {
  ...
  data () {
    ...
    return {
      ...
      subscriptionRules: {
        "subscriptionName": [{ required: true, message: this.$t('seaProduct.placeholder2'), trigger: 'change' }],
        "cycleWeekList": [{ required: true, message: this.$t('seaProduct.placeholder'), trigger: 'change' }],
    ...
    addOrder () {
      ...
      if (ve && vb) {
        ...
        this.subscriptionMode.chargeForm.charges.forEach(element => {
          ...
          console.log(element.podJsonList, element.routeCode)
        ...
        saveSeaProductSubscriptionApi(this.subscriptionMode).then(res => {
          if (res && res.ok) {
