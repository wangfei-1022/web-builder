<template>
  <div>
    <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column prop="chargeItemCurrency" :label="$t('seaProduct.currency')" width="150">
        <template slot-scope="{row, $index}">
          <el-select v-if="!isDetail" v-model="row.chargeItemCurrency" size="mini"
            @change="changeCurrency($index, $event)">
            <el-option v-for="item in currencyList" :key="item.code" :label="item.code" :value="item.code" />
          </el-select>
          <span v-if="isDetail">{{ row.chargeItemCurrency }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="price" :label="$t('seaProduct.price')" width="100">
        <template slot-scope="{ row, $index }">
          <el-input v-if="!isDetail" v-model="row.price" :placeholder="$t('seaProduct.please_enter')" size="mini"
            ...
            oninput="value=value.replace(/^\D*(\d*(?:\.\d{0,3})?).*$/g,'$1')"></el-input>
...
export default {
    ...
    chargeTypeItem: {
      handler (val, oldVal) {
        ...
        data.forEach(item => {
          ...
          obj.chargeItemCurrency = item.chargeItemCurrency
  ...
  methods: {
    ...
    changeCurrency (index, currency) {
      let mockData = this.chargeTypeItem.chargeItemDetailDTOList
      mockData[index].chargeItemCurrency = currency
      console.log(index, currency, this.tableData, this.categoryId)
      this.$emit('update', this.tableData, this.categoryId)
    }
  }
}
