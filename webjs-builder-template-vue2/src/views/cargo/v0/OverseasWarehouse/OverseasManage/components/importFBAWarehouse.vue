<template>
	<div>
		<el-dialog
			...
			:title="$t('overseasManage.batchFBAWarehouse')"
			width="550px"
			:close-on-click-modal="false"
		>
      <div class="dialog-content">
        <el-form label-width="0px" label-position="left" :model="formData" ref="formData" v-if="dialogVisible" v-loading="loading" >
            ...
            >
              <div slot="upload">
                <el-button size="mini" type="primary"  class="ml-10" :loading="loading">
                  {{ $t("common.upload.uploadClickTitle") }}
                </el-button>
              </div>
            </UploadFile>
            <span>{{ $t("common.upload.uploadXlsx") }}</span>
            <span class="fc-blue ml-10 pointer" @click="downloadExcel('FBA仓库导入模板.xlsx')">{{$t("common.upload.downloadTemplate")}}</span>
          </el-form-item>
        </el-form>
      </div>
...
<script>
import { exportFBAWarehouseApi } from "@/api/OverseasWarehouse/overseasManage"
export default {
	name: 'ImportFBAWarehouse',
	props: {
		visible: {
			type: Boolean,
			default: false
...
*/
		...
		async saveForm() {
			...
			this.loading = true
			let rs = await exportFBAWarehouseApi(this.formData)
			if (rs.ok) {
				this.loading = false
