	...
	<div v-loading="loading" class="pd-16">
		...
		>
			<template slot="multipleSearch">
				<el-form
						...
						label-width="100px"
					>
						<el-form-item :label="$t('channelPriceCard.customer_name')" prop="customerId">
      ...
      :style="gridHeight"
		...
		<Pagination
			...
			@pagination="list"
		/>
		<CustomerPriceCardDialog :visible.sync="dialog.visible" />
...
import CustomerPriceCardDialog from './components/CustomerPriceCardDialog.vue'
import operationBtn from '../components/OperationBtn.vue'
import { getProductListApi } from '@/api/OverseasWarehouse/overseasWarehouseManager'
import { getPriceCardsOfCustomerApi, publishCostPriceCard, publishTruckCostPriceCard, delPriceCard } from '@/api/OverseasWarehouse/PriceCardManage.js'
...
export default {
	...
	components: { operationBtn, NationalGroupSelect, CustomerPriceCardDialog, cargoLimitedSelect, CopyPriceDialog, PostponeDialog },
	data() {
		return {
			formData: {
				customerId: null,
			...
			columnDefsRight: [
				{
					headerName: this.$t('channelPriceCard.customer_name'),
					field: 'customerName'
				},
				{
	 ...
	 */
		...
		async list() {
			...
			const res = await getPriceCardsOfCustomerApi(this.formData).catch(() => {
				this.loading = false
			})
			if (res.ok) {
		...
		*/
		...
		add(id, type, edit, productType) {
			if (id) {
				edit = edit || 0
				this.$router.push({ name: 'ProductCustomerPriceCardDetail', query: { type, id, edit, productType, time: new Date().getTime() } })
			} else {
