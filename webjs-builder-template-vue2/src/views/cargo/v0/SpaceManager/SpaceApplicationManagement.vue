  ...
  <div class="sys-right-inner-wrap" v-loading="loading">
    <top-search-items-slot :searchFun="topSearch" :resetFun="clearSearchForm">
      <el-form slot="multipleSearch" ref="multipleForm" :inline="true" :model="multipleForm" label-width="80px" size="mini" @submit.native.prevent>
        ...
        <el-form-item :label="`${$t('spaceRule.shippingSpace.applicant')}: `" prop="routeCode">
          <cargo-limited-select
            placeholder=""
            :select-label="multipleForm.chineseName"
            :select-value="multipleForm.applyOwner"
            type="user"
            style="width: 100%;"
            clearable
            @set="(data)=>{
              multipleForm.applyOwner = data.id
              topSearch()
            }"
          />
        </el-form-item>
    ...
    <div class="container-wrap table-wrap">
      <div class="top-btn-wrap mb-10">
        <el-button type="primary" size="mini" class="ml-10" @click="exportApply">{{ $t('spaceRule.shippingSpace.exportApplicationStatistics') }}</el-button>
      </div>
      <ag-grid-vue
...
import {
  addContractApi, applyManageApi, applyProExportlApi,
  applyProPagesApi,
  applySaleApi,
  applySaleDelApi,
  applySaleDetailApi,
...
import {payDetailExportApi} from "@/api/seaExportOrder";
export default {
    ...
    'incomeComponent': {
      template: `<div style="font-size: 14px">
        <el-button type="text" size="mini" :disabled="true" @click="params.context.componentParent.editRow(params.data)">${Vue.$t('spaceRule.shippingSpace.distribution')}</el-button>
        <el-button  type="text" size="mini" :disabled="true" @click="params.context.componentParent.deleteContract(params.data)">${Vue.$t('spaceRule.shippingSpace.operationLog')}</el-button>
        </div>
      `,
      methods: {}
    }
  ...
  watch: {
    ...
    calculationTeu(val) {
      ...
      this.totalTeu = total
		...
		async getList() {
			...
			this.selectedItems = []
			this.loading = true
			const res = await applyProPagesApi(data)
			this.loading = false
			if (res.ok) {
    ...
    async exportApply() {
      this.loading = true
      const res = await applyProExportlApi(this.multipleForm)
      this.loading = false
      if (res.ok) {
    ...
    this.columnDefsRight = [{
      ...
      headerName: this.$t('spaceRule.shippingSpace.applicationTime'), // 申请人
      field: 'createByName',
      minWidth: 100
    }, {
