<template>
  <el-dialog :visible.sync="allRecordedDialog" v-if="allRecordedDialog" :title="$t('sea.expenseDetailsFiltering')" width="450px" :before-close="closeDialog" v-loading="loading">
    <el-form v-if="allRecordedType" ref="dataForm" :model="dataForm" :inline="false" class="dataForm" style="margin-top: 10px;">
      <el-form-item :label="$t('sea.customerName')+':'" prop="customerName" size="mini"  :rules="[ { required: true,message:$t('sea.placeholder'),trigger: 'change'} ]">
        <el-select v-model="dataForm.customerName" @change="customerNameChange" size="mini" filterable clearable style="width: 250px" >
          <el-option v-for="item in allRecordedDialogData" :key="item.id" :label="item.name" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :label="$t('sea.settlementUnit')+':'" prop="settlementUnit" size="mini"  :rules="[ { required: true,message:$t('sea.placeholder'),trigger: 'change'} ]">
        <el-select v-model="dataForm.settlementUnit" @change="settlementUnitChange" size="mini" filterable clearable style="width: 250px" >
          <el-option v-for="item in settlementUnitOps" :key="item.id" :label="item.name" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :label="$t('sea.settlementMethod')+':'" prop="settlementMethod" size="mini"  :rules="[ { required: true,message:$t('sea.placeholder'),trigger: 'change'} ]">
        <el-select v-model="dataForm.settlementMethod" @change="settlementMethodChange" size="mini" filterable clearable style="width: 250px" >
    ...
    <div class="footer-btn">
      <el-button type="primary" size="small" @click="next">{{ $t('sea.next') }}</el-button>
...
.dataForm{
  ...
  ::v-deep .el-form-item__error{ margin-left: 75px}
}
</style>
