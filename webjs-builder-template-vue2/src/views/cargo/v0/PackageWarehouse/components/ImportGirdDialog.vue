...
import { uploadGirdProductApi, uploadGirdSortApi } from '@/api/packageWarehouse'
import { getToken } from '@/utils/auth'
export default {
  name: 'ImportGirdDialog',
  props: {
    title: {
      type: String,
      default: ''
  ...
  methods: {
    ...
    downloadExcel() {
      ...
      // link.href = encodeURI(`${process.env.VUE_APP_BASE_API}/api/mdp-service/template/excel/download?templateName=${_fileName}&Authorization=${getToken()}`)
    ...
    async saveForm() {
      this.loading = true
      let res = null
      if (this.importType === 'sort') {
        res = await uploadGirdSortApi(this.goodsInfoExcelCloudFileVo)
      } else {
        res = await uploadGirdProductApi(this.goodsInfoExcelCloudFileVo)
      }
      if (res.ok) {
        this.dialogVisible = false
        this.$message.success(this.$t('packageWarehouse.girdConfig.importSuccess'))
