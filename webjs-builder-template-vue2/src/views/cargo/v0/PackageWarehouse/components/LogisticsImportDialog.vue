...
<script>
import { importLogisticsApi } from '@/api/packageWarehouse'
import { getToken } from '@/utils/auth'
export default {
  name: 'LogisticsImportDialog',
  components: {},
  props: {
    visible: {
      type: Boolean,
  ...
  methods: {
     ...
     */
    downloadExcel() {
      this.downloadFile({ fileName: this.$t('packageWarehouse.logisticManager.importFileName'), url: encodeURI(`${process.env.VUE_APP_BASE_API}/api/mdm-service/template/excel/download?templateName=${this.$t('packageWarehouse.logisticManager.importFileName')}&Authorization=${getToken()}`) })
    },
    /**
     * 文件超出个数限制时的钩子
     */
    ...
    async submitForm() {
      ...
      const res = await importLogisticsApi(this.fileObj)
      if (res && res.ok) {
