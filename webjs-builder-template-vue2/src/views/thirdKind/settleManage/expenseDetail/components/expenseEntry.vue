<template>
  <el-dialog :visible.sync="expenseEntryDialog" v-if="expenseEntryDialog" :title="$t('sea.expenseEntry')" width="1000px"
    :before-close="closeexpenseEntryDialog" v-loading="loading">
    <div>{{ $t('sea.pleaseCheckTheNeedTo') }}<span
        style="font-weight: 700;color: rgba(255,0,0,0.65); font-size: 16px">{{ dataForm.confirmationNo }}</span>{{
            $t('sea.transferInTheSelectedConfirmation') }}</div>
    <el-form ref="dataForm" :model="dataForm" :inline="true" class="dataForm" style="margin-top: 10px;">
      <el-form-item :label="$t('sea.entryMethod') + ':'" prop="entryMethod" size="mini"
        :rules="[{ required: true, message: $t('sea.placeholder'), trigger: 'change' }]">
      ...
      <el-form-item v-if="!detailedType && initialData.memberName" :label="$t('sea.customerName') + ':'"
        prop="blSignType" size="mini" class="shipping_method">
        {{ initialData.memberName }}
      </el-form-item>
      <el-form-item v-else
        :label="detailedType ? $t('sea.payableSettlementUnit') + ':' : $t('sea.aRSettlementUnit') + ':'"
        prop="blSignType" size="mini" class="shipping_method">
        {{ initialData.settlementCompanyName }}
      </el-form-item>

      <!--        账单周期-->
      <el-form-item v-if="dataForm.entryMethod === 1" :label="$t('sea.billingCycle') + ':'" prop="periodCode"
        size="mini" :rules="[{ required: true, message: $t('sea.placeholder'), trigger: 'change' }]"
        class="shipping_method">
        <el-select v-model="dataForm.periodCode" size="mini" filterable clearable>
          <el-option v-for="item in billPeriodOps" :key="item.periodCode" :label="item.periodName"
            :value="item.periodCode"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item v-else-if="dataForm.entryMethod === 2" :label="$t('sea.billNumber') + ':'" prop="billNo" size="mini"
        :rules="[{ required: true, message: $t('sea.placeholder'), trigger: 'change' }]" class="shipping_method">
      ...
      <!--    客户结算单位-->
      <el-form-item v-if="!detailedType && initialData.memberName"
        :label="detailedType ? $t('sea.payableSettlementUnit') + ':' : $t('sea.aRSettlementUnit') + ':'"
        prop="blSignType" size="mini" class="shipping_method">
        {{ initialData.settlementCompanyName }}
      </el-form-item>
...
<script>
import { entryApChargeApi, entryArChargeApi, detailApChargeBillApi, detailArChargeBillApi } from "@/api/thirdKind"
import { getPeriodListApi } from "@/api/financial/fmsReceive"
...
export default {
	...
	created() {
		getPeriodListApi(this.initialData.settlementMode).then(res => {
			this.billPeriodOps = res.content
		})
		if (this.detailedType) {
			detailApChargeBillApi(this.initialData).then(res => {
				this.billNoOps = res.content
			})
		} else {
			detailArChargeBillApi(this.initialData).then(res => {
				this.billNoOps = res.content
			})
		}
		...
		async submit() {
			this.$refs.dataForm.validate(async valid => {
				if (valid) {
					...
					// } else { // 选择入账
					// 	if (this.detailedType) {
					// 		url = entryApChargeApi
					// 	} else {
					// 		url = entryArChargeApi
					// 	}
					// 	data = {
					// 		...this.dataForm,
					// 		...this.initialData
