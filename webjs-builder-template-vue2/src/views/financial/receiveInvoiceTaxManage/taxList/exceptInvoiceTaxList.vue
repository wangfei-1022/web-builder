	...
	<div class="right-main-wrapper" v-loading="loading">
		...
		<div class="">
			<div class="top-btn-wrap mb-10 mt-10">
				<!--异常-->
				<!-- <el-button size="mini" type="primary" @click="submitTaxFn" v-if="invoiceType != 3 && invoiceState == 'EXCEPT'">重新提交</el-button> -->
			</div>

			<ag-grid-vue-pro :data="dataList" :style-grid="gridHeight" :column-fields-default="columnFields" :grid-ready="onReady" :is-no-oper="true" />
...
import exceptInvoiceTaxListTable from "../table/exceptInvoiceTaxListTable";
import { parseTime, formatNumberToThousands, downloadFile } from '@/utils/index'
import { getTaxInvoiceExceptListByFinanceApi, getTaxInvoiceExceptTotalByFinanceApi } from "@/api/financial/fmsReceiveInvoice"
...
export default {
	...
	created() {
		this.getList();
		this.getBaseDataFn();
		this.columnFields = exceptInvoiceTaxListTable;
	},
	methods: {
		onReady(params) {
    ...
    async getList() {
      ...
      const res = await getTaxInvoiceExceptListByFinanceApi(params)
      if (res.ok) {
      ...
      getTaxInvoiceExceptTotalByFinanceApi(params).then(res => {
        if (res.ok) {
