...
import uploadInvoiceFileCell from "../components/uploadInvoiceFileCell"; // 发票扫码件
import uploadChargeFileCell from "../components/uploadChargeFileCell"; // 费用明细扫描件
import uploadOtherFileCell from "../components/uploadOtherFileCell"; // 其它附件扫描件
import touploadInvoiceTaxListTable from "../table/touploadInvoiceTaxListTable";
import { parseTime, formatNumberToThousands, downloadFile } from '@/utils/index'
import { getTaxInvoiceTouploadListByFinanceApi, getTaxInvoiceTouploadTotalByFinanceApi} from "@/api/financial/fmsReceiveInvoice"
...
export default {
	...
	created() {
		this.columnFields = touploadInvoiceTaxListTable;
	},
  methods: {
    async getList() {
      // 待回传
      ...
      const res = await getTaxInvoiceTouploadListByFinanceApi(params)
      if (res.ok) {
      ...
      getTaxInvoiceTouploadTotalByFinanceApi(params).then(res => {
        if (res.ok) {
