...
import sendExceptinvoiceTaxListTableFinance from "../table/sendExceptinvoiceTaxListTableFinance";
import {
  resendTaxInvoiceApi, // 重新推送
  getTaxInvoiceSendExceptListByFinanceApi, // 税票-财务-异常
  getTaxInvoiceSendExceptTotalByFinanceApi,
...
export default {
	...
	created() {
		this.columnFields = sendExceptinvoiceTaxListTableFinance;
	},
  methods: {
    resendTaxInvoiceFn() {
      ...
      resendTaxInvoiceApi(data).then(res => {
        if (res.ok) {
    ...
    async getList() {
      ...
      const res = await getTaxInvoiceSendExceptListByFinanceApi(params)
      if (res.ok) {
      ...
      getTaxInvoiceSendExceptTotalByFinanceApi(params).then(res => {
        if (res.ok) {
