  ...
  <div class="clear-charge-detail">
    <div class="section" style="padding: 0 20px 20px 20px;">
      ...
      <el-table size="mini" :data="tableData" style="width: 100%;" row-key="id" border>
        ...
        <el-table-column  prop="chargeCurrency" :label="$t('fmsReceiveClear.clearChargeCurrencyAmount')" width="120">
          <template slot-scope="scope">
            <div style='text-align: right'>
              {{scope.row.clearCurrencyAmountsStr}}
            </div>
          </template>
        </el-table-column>
...
export default {
	...
	methods: {
      ...
      if (this.type === "CLAIMER") {
        ...
        totalFn = getClaimClearInfoTotalApi
        data.claimId = this.id
      } else {
        fn = getTransactionClearInfoApi;
        totalFn = getTransactionClearInfoTotalApi
