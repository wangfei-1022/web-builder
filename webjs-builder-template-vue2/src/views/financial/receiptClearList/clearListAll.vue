		...
		<top-search-items-slot :searchFun="getList" :resetFun="reset">
			<template slot="multipleSearch">
              ...
              <base-select v-model="formData.settlementOfficeId" type="settlementOffice" @selected="getList" addAll></base-select>
						...
						<el-form-item :label="$t('fmsReceiveClear.clearBy')" prop="clearById">
...
<script>
import { getClearListAllApi, getClearListAllToclearApi, receiptClearCancelApi, receiptApplyClearCancelApi } from "@/api/financial/fmsReceiptClear";
		...
		return {
			...
			columnFields: [
				...
				},
				{
					...
					headerName: this.$t('fmsReceiveClear.settlementCompany'),
					minWidth: 100
				},
				{
					field: "settlementOfficeName",
					headerName: this.$t('fmsReceiveClear.settlementOffice'),
					minWidth: 200
				},
				{
              ...
              if (v.sourceValue) {
				...
				{
					field: "clearByName",
					headerName: this.$t('fmsReceiveClear.clearBy'),
					minWidth: 100
				},
				{
					field: "clearDate",
	...
	methods: {
		...
		async getList() {
			...
			if (this.role === "BUSINESS") {
				fn = getClearListAllToclearApi;
			} else {
				fn = getClearListAllApi;
			}
		...
		clearCancelFn(item) {
			this.$confirm(this.$t('fmsReceiveClear.recallClearInfo'), this.$t("common.mention"), {
				...
				.then(() => {
          this.loading = true
					receiptClearCancelApi({ clearId: item.clearId }).then(res => {
						if (res.ok) {
							this.getList();
