  ...
  <el-dialog :visible.sync="expenseEntryDialog" v-if="expenseEntryDialog" :title="$t('sea.expenseEntry')" width="1000px"
    ...
    <div>{{ $t('sea.pleaseCheckTheNeedTo') }}<span
        style="font-weight: 700;color: rgba(255,0,0,0.65); font-size: 16px">{{ dataForm.confirmationNo }}</span>{{
            $t('sea.transferInTheSelectedConfirmation')
        }}</div>
...
import {
  detailAllEntryApi, detailAllEntryChargeApi, detailListEntryApi, detailListEntryChargeApi, periodSettlementModeApi,
  supplierHistoryApi, supplierHistoryChargeApi
} from "@/api/airExport";
...
export default {
  ...
  created () {
    ...
    periodSettlementModeApi(this.initialData.settlementMode).then(res => {
