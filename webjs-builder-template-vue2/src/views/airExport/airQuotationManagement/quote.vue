  ...
  <div class="app" v-loading="loading">
    <div style="margin-left:10px;margin-top: 14px;">
                ...
                <el-col :span="4">
                  <el-form-item :label="$t('air.customerSplitBubbleHead')" prop="firstVolumetricFlag">
                    <el-checkbox v-model="firstVolumetricFlag" @change="firstVolumetricFlagChange">
                      <span style="font-size:13px">{{ firstVolumetricFlag ? $t('air.yes') : $t('air.no')
                      }}</span></el-checkbox>
                  </el-form-item>
                </el-col>
                <el-col :span="20">
      ...
      <div class="operation-btn">
        <el-button v-if="orderDetail.baseDTO.orderState === 4" type="danger" @click="cancellationOfOrder">{{
...
import { getAirExportOrderDetailApi, addAirOrderOffer, cancelAirExportApi } from '@/api/airExport'
  ...
  methods: {
    ...
    getDetail () {
      ...
      getAirExportOrderDetailApi(id).then(res => {
        if (res.content) {
          ...
          this.order.firstVolumetric = res.content.firstVolumetric // 头程分泡值
          if (res.content.firstVolumetric) {
            this.firstVolumetricFlag = true // 是否头程分泡
          } else {
            this.firstVolumetricFlag = false // 是否头程分泡
          }

          if (res.content.lastVolumetric) {
            this.lastVolumetricFlag = true // 是否尾程分泡
          } else {
            this.lastVolumetricFlag = false // 是否尾程分泡
          }
          this.order.lastVolumetric = res.content.lastVolumetric // 尾程分泡值

          if (res.content.serviceItem) {
            this.order.serviceItemList = res.content.serviceItem.split(",") // 服务项多个逗号隔开:1、自报自交2、拖车服务3、报关服务
          } else {
            this.order.serviceItemList = []
          }

          this.order.specialFlag = res.content.specialFlag // 是否特价
          this.order.fileList = []
          // 特价文件处理
          ...
          console.log(this.order.fileList)
          // this.order.airOrderExportChargeDtoList = res.content.airOrderExportChargeDtoList // 成本费用列表

          this.order.associatedCostPrice = res.content.associatedCostPrice // 关联成本价
          this.order.chargeForm.charges = []
          if (res.content.airOrderExportChargeDtoList.length > 0) {
            res.content.airOrderExportChargeDtoList.forEach(v => {
              try {
                // 应收
                if (v.arAp === 1) {
                  this.order.chargeForm.charges.push(v)
                }
              } catch (e) {
                console.log(e)
              }
          ...
          if (this.order.chargeForm.charges.length === 0) {
            // 起运港=PVG，30，=CAN，50，=NTG，20
            let polPrice = ""
            if (this.orderDetail.spaceDTO.polCode === 'PVG') {
              polPrice = 30
            } else if (this.orderDetail.spaceDTO.polCode === 'CAN') {
              polPrice = 50
            } else if (this.orderDetail.spaceDTO.polCode === 'NTG') {
              polPrice = 20
            }
    ...
    addOrder () {
      ...
      if (ve && vb) {
        ...
        this.order.orderIdList.push(this.$route.query.id)
    ...
    uploadSuccess (res) {
      ...
      // this.order.fileList.push(res.content)
