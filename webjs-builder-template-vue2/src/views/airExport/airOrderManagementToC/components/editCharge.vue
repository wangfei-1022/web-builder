...
export default {
  ...
  methods: {
    init () {
      Promise.all([getDictDataApi(8), getTaxRateListApi(), getCurrenciesListApi(), getYHTListApi(), getCustomerDropDownListApi(), rechargeModeApi(), airCustomerTitleList({ businessTime: this.etd, businessType: 'AIR_EXPORT', memberId: this.memberId })]).then(results => {
