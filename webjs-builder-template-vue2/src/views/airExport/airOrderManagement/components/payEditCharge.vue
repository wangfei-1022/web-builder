        ...
        <el-row>
          <!-- <el-col :span="6">
            <el-form-item :label="$t('air.settlementUnitType')" prop="partnerRole"
              ...
              <el-select v-model="editForm.partnerRole" size="mini" :disabled="item.confirmationState === 11" @change="partnerRoleChange">
                ...
                <el-option :label="$t('air.customsBroker')" :value="16010"></el-option>
          ...
          <el-col :span="6">
            ...
            <el-form-item :label="$t('air.supplier')"
              :rules="[{ required: true, message: $t('receiveFms.can_not_be_null') }]" prop="scmSupplierCnName">
                <!-- <el-select v-model="editForm.scmSupplierCnName" clearable filterable size="mini"
                :disabled="item.confirmationState === 11" @change="supplierChange">
                <el-option v-for="item in supplierTitleList" :key="item.id"
                  :label="item.chineseName" :value="item.id">
                </el-option>
              </el-select> -->
          ...
          <el-col :span="6">
            ...
            <el-form-item :label="$t('air.settlementMethod')" prop="settlementModeName"
              ...
              <el-select v-model="editForm.settlementModeName" size="mini" :disabled="item.confirmationState === 11" @change="settlementModeChange">
                <el-option v-for="(item,index) in settlementModeOps" :key="index" :label="item.name" :value="item.code">
                </el-option>
              </el-select>
            </el-form-item>
          </el-col>
...
import {
  dmnBusinessList, supplierTypeBusinessApi, supplierSupplierTypeApi, supplierSupplierActiveTitleApi
} from "@/api/airExport"
...
export default {
  ...
  watch: {
    item: {
      handler (val) {
        ...
        // this.partnerRoleChange(this.editForm.partnerRole, "init")
      },
      immediate: true
    }
  },
  ...
  methods: {
      ...
      Promise.all([getDictDataApi(8), getTaxRateListApi(), getCurrenciesListApi(), getYHTListApi(), getCustomerDropDownListApi(), rechargeModeApi()]).then(results => {
        ...
        if (YHTRes.ok) {
          ...
          if (this.type === "ADD") {
            ...
            // this.editForm.partnerRole = 12015
            // this.partnerRoleChange(this.editForm.partnerRole)
          }
        }
        if (settlementRes.ok) {
    ...
    async supplierChange(item) {
      ...
      // let supplieTitle = this.supplierTitleList.find(res => item.id === res.id)
      // if (supplieTitle) {
      //   this.editForm.scmSupplierCnName = supplieTitle.chineseName
      // }
      let param = {
        "memberId": item.id,
    ...
    // 选择结算单位类型
    // async partnerRoleChange(item, type) {
    //   if (type !== "init") {
    //     this.editForm.scmSupplierId = ""
    //     this.editForm.scmSupplierCnName = ""
    //     this.supplierTitleList = []
    //     this.editForm.settlementCompanyId = ""
    //     this.editForm.settlementCompanyName = ""
    //     this.editForm.settlementMode = ""
    //     this.editForm.settlementModeName = ""
    ...
    //   }
    //   if (item) {
    //     const supplierTypeRes = await supplierTypeBusinessApi("AIR_EXPORT");
    //     if (supplierTypeRes && supplierTypeRes.ok) {
    //       this.supplierTitleList = supplierTypeRes.content
    //     }
    //   }
    // },
    calcTotalFn () {
