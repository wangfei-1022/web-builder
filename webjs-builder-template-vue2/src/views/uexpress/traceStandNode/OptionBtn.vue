<template>
  <div class="option-btn-wrap">
    <el-button type="text" size="mini" @click="modifyTrace">
      {{$t("common.modifyBtn")}}
    </el-button>
...
<script>
import { deleteStandNodeApi } from '@/api/uexpress/traceLine'
export default {
  ...
  methods: {
    modifyTrace() {
      this.params.modifyTrace(this.params.data)
    },
    checkdelete() {
    ...
    async deleteTrace() {
      const res = await deleteStandNodeApi({trackingNodeId: this.params.value})
      if (res.ok) {
