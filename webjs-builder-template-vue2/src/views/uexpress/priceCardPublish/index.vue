  ...
  <div class="right-main-wrapper" v-loading="loading">
        ...
        <div class="multiple-form-wrap">
          <el-form ref="formData" :inline="true" :model="formData"  size="mini" label-width="100px" class="contract-search">
            ...
            <el-form-item :label='$t("uexpressPriceCard.productType")' prop="productType">
							<uexpress-select v-model="formData.productType" type="productType" style="width: 100%" addALL  @selected="getList" />
            </el-form-item>
    ...
    <div class="table-wrap">
      <div style="padding: 0px 0 10px 0;" class="top-btn-wrap">
        <el-button type="primary" size="mini" icon="el-icon-plus" @click="addFn" v-if="role === 'PRODUCT'">{{$t("uexpressPriceCard.add")}}</el-button>
        <el-button type="primary" size="mini" @click="uploadFn" v-if="role === 'PRODUCT'">{{$t("uexpressPriceCard.uploadPriceCard")}}</el-button>
        <el-button type="primary" size="mini" @click="batchSubmitFn" v-if="role === 'PRODUCT'">{{$t("uexpressPriceCard.batchSubmitPrice")}}</el-button>
        <el-button type="primary" size="mini" @click="batchPublishFn" v-if="role === 'PRODUCT'">{{$t("uexpressPriceCard.batchPublishPrice")}}</el-button>
        <el-button type="primary" size="mini" @click="exportFn" v-if="role === 'PRODUCT'">{{$t("uexpressPriceCard.export")}}</el-button>
      </div>
      ...
      >
        ...
        <el-table-column prop="offerName" :label='$t("uexpressPriceCard.offerName")' min-width="130" show-overflow-tooltip></el-table-column>
        ...
        <el-table-column fixed="right" prop="operation" :label='$t("uexpressPriceCard.operation")' min-width="240" v-if=" role === 'PRODUCT' ">
          <template slot-scope="{row}">
        ...
        <el-table-column fixed="right" prop="operation" :label='$t("uexpressPriceCard.operation")' min-width="100" v-if=" role === 'SALE' ">
          <template slot-scope="{row}">
            <a class="link" @click="viewFn(row)">{{$t("uexpressPriceCard.view")}}</a>
          </template>
        </el-table-column>
    ...
    <el-dialog :visible.sync="batchImportVisible" :close-on-click-modal="false" width="500px" :title='$t("uexpressPriceCard.uploadPriceCard")'>
      <el-form class="contact-form" ref="batchImportRef" :model="importForm" :rules="importFormRule" label-width="100px" size="mini">
        <div style="text-align: center;margin-bottom: 20px;">
          <el-button size="mini" type="primary" icon="el-icon-download" @click="downloadTemplateFn(21)">{{$t("uexpressPriceCard.uploadPriceCardTemplate")}}</el-button>
        </div>
...
<script>
import {
  getOfferPublishProductListApi,
  getOfferPublishSaleListApi,
  getOfferPublishHistoryListApi,
  submitOfferPublishApi,
  publishOfferPublishApi,
  deleteOfferPublishApi,
  adjustInitOfferPublishApi,
  importOfferPublishApi,
  exportOfferPublishApi,
  offlineExpireTimeOfferPublishApi // 下线价卡
} from '@/api/uexpress/priceCardPublish'
...
export default {
	name: 'UexpressPriceCardPublishReceipt',
	props: {
		role: {
			type: String,
			default: ''
  ...
  activated() {
    ...
    checkPublishFn(row) {
      return row.offerState === 30 && this.role === 'PRODUCT'
    },
    checkSubmitFn(row) {
      return row.offerState === 20 && this.role === 'PRODUCT'
    },
    exportFn() {
      this.loading = true
      let params = this.getParams()
      exportOfferPublishApi(params).then(res => {
        if (res.ok) {
    ...
    batchImportConfirmFn() {
      this.$refs.batchImportRef.validate(valid => {
        if (valid) {
          ...
          importOfferPublishApi(data).then(res => {
            if (res.ok) {
    ...
    getParams() {
      ...
      let fn = null
      if (this.role === 'SALE') {
        fn = getOfferPublishSaleListApi
      } else {
        fn = getOfferPublishProductListApi
      }
      ...
      if (this.role === 'SALE') {
        let arr = []
        tree.historyVersions.forEach(function (v, index) {
        ...
        return
			...
			getOfferPublishHistoryListApi(tree.offerId).then(res => {
				let arr = []
				if (res.content) {
    ...
    submitFn(row) {
      this.$confirm(this.$t("uexpressPriceCard.submitInfo"), this.$t('common.mention'), {
        ...
        this.loading = true
        const res = await submitOfferPublishApi(data)
        this.loading = false
        if (res.ok) {
          this.getList()
    ...
    adjustFn(row) {
      this.$confirm(this.$t("uexpressPriceCard.adjustInfo"), this.$t('common.mention'), {
        ...
        this.loading = true
        const res = await adjustInitOfferPublishApi(data)
        this.loading = false
        if (res.ok) {
          this.$router.push({
    ...
    publishFn(row) {
      this.$confirm(this.$t("uexpressPriceCard.publishInfo"), this.$t('common.mention'), {
        ...
        this.loading = true
        const res = await publishOfferPublishApi(data)
        this.loading = false
        if (res.ok) {
          this.getList()
    ...
    deleteFn(row) {
      this.$confirm(this.$t("uexpressPriceCard.deleteInfo"), this.$t('common.mention'), {
        ...
        this.loading = true
        const res = await deleteOfferPublishApi(data)
        this.loading = false
        if (res.ok) {
          this.getList()
    ...
    offlineFn(row) {
      this.$confirm(this.$t("uexpressPriceCard.offlineInfo"), this.$t('common.mention'), {
        ...
        this.loading = true
        offlineExpireTimeOfferPublishApi(data).then(res => {
          if (res.ok) {
