<template>
  <div class="right-main-wrapper" v-loading="loading">
    ...
    <el-dialog :visible.sync="batchImportVisible" :close-on-click-modal="false" width="500px" :title='$t("uexpressOrder.importDeclareImport")'>
      <el-form class="contact-form" ref="batchImportRef" :model="importForm" :rules="importFormRule" label-width="100px" size="mini">
        <div style="text-align: center;margin-bottom: 20px;">
          <el-button size="mini" type="primary" icon="el-icon-download" @click="downloadTemplateFn">{{$t("uexpressOrder.importDeclareImportTemplate")}}</el-button>
        </div>
...
import {
  ...
  importDeclareImportApi, // 清关查验-导入
  importDeclareSupplierDestructionApi, // 清关查验-供应商销毁
...
export default {
  ...
  data() {
    return {
      ...
      columnFields: [{
        ...
        {
          field: "bolNo",
          headerName: this.$t("uexpressOrder.bolNo"),
          minWidth: 120
        },
        {
  ...
  methods: {
    // 导入
    downloadTemplateFn() {
      getTemplateFileApi(14).then(res => {
    ...
    batchImportConfirmFn() {
      this.$refs.batchImportRef.validate(valid => {
        if (valid) {
          ...
          importDeclareImportApi(data).then(res => {
            if (res.ok) {
    ...
    supplierDestructionFn() {
      ...
      this.$confirm(this.$t("uexpressOrder.supplierDestructionMention", { num: ids.length }), this.$t("common.mention"), {
        ...
        importDeclareSupplierDestructionApi(data).then(res => {
          if (res.ok) {
            this.getList()
