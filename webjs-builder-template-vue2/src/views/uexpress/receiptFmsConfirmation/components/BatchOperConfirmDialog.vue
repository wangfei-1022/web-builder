<template>
    <el-dialog :title="title" :visible.sync="visible" width="450px" @close="closeFn">
      <div v-if="state === 1">
        {{$t('uexpressFmsConfirmation.batchOperInfo', { count: allIds.length, oper: title })}}
      </div>
      <div v-if="state === 2">
        {{$t('uexpressFmsConfirmation.batchOperRuningInfo', { inCount: (allIds.length - ids.length), unCount: ids.length })}}
      </div>
      <div v-if="state === 4">
        {{$t('uexpressFmsConfirmation.batchOperEndingInfo', { successCount: successIds.length, failCount: failIds.length })}}
      </div>
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" size="mini" :loading="true" v-if="state === 2">{{$t('uexpressFmsConfirmation.excuteRuning')}}</el-button>
  ...
  beforeDestroy() {
    ...
    startFn() {
      ...
      this.submitFn()
		...
		async submitFn() {
      if (this.ids.length) {
        let data = this.ids.shift()
        const res = await this.api(data)
        this.submitFn()
        if (res.ok) {
          this.successIds.push(data)
        } else {
          this.failIds.push(data)
        }
      } else {
        this.state = 4
        window.removeEventListener('beforeunload', this.refreshFn);
