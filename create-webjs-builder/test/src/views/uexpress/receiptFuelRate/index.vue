  ...
  <div class="right-main-wrapper" v-loading="loading">
    ...
    <div class="table-wrap">
      <div style="padding: 0px 0 10px 0;" class="top-btn-wrap">
        ...
        <el-button type="primary" size="mini" @click="uploadFn" v-if="false">{{$t("uexpressPriceCard.uploadPriceCard")}}</el-button>
      ...
      >
        ...
        <el-table-column prop="productName" :label='$t("uexpressPriceCard.productName")' min-width="180" show-overflow-tooltip>
          <template slot-scope="{row}">
            <span :class="!row.hasChildren && !row.isChildren ? 'offerNoLeft' : ''">{{row.productName}}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog :visible.sync="batchImportVisible" :close-on-click-modal="false" width="500px" :title='$t("uexpressPriceCard.uploadPriceCard")'>
      <el-form class="contact-form" ref="batchImportRef" :model="importForm" :rules="importFormRule" label-width="100px" size="mini">
        <div style="text-align: center;margin-bottom: 20px;">
          <el-button size="mini" type="primary" icon="el-icon-download" @click="downloadTemplateFn(28)">{{$t("uexpressPriceCard.uploadPriceCardTemplate")}}</el-button>
        </div>
...
<script>
import {
  getReceiptFuelRateListApi,
  getReceiptFuelRateHistoryListApi,
  publishReceiptFuelRateApi,
  deleteReceiptFuelRateApi,
  importReceiptFuelRateApi,
  exportReceiptFuelRateApi,
  offlineExpireTimeReceiptFuelRateApi // 下线价卡
...
export default {
  ...
  activated() {
    ...
    exportFn() {
      ...
      exportReceiptFuelRateApi(params).then(res => {
        if (res.ok) {
    ...
    batchImportConfirmFn() {
      this.$refs.batchImportRef.validate(valid => {
        if (valid) {
          ...
          importReceiptFuelRateApi(data).then(res => {
            if (res.ok) {
    ...
    getParams() {
      ...
      return data
		...
		async getList() {
			...
			let params = this.getParams()
      let fn = null
			const res = await getReceiptFuelRateListApi(params)
			if (res.ok) {
      ...
      this.loading = false
		...
		loadFn(tree, treeNode, resolve) {
			getReceiptFuelRateHistoryListApi(tree.offerId).then(res => {
				let arr = []
				if (res.content) {
    ...
    publishFn(row) {
      this.$confirm(this.$t("uexpressPriceCard.publishInfo"), this.$t('common.mention'), {
        ...
        this.loading = true
        const res = await publishReceiptFuelRateApi(data)
        this.loading = false
        if (res.ok) {
          this.getList()
    ...
    deleteFn(row) {
      this.$confirm(this.$t("uexpressPriceCard.deleteInfo"), this.$t('common.mention'), {
        ...
        this.loading = true
        const res = await deleteReceiptFuelRateApi(data)
        this.loading = false
        if (res.ok) {
          this.getList()
    ...
    offlineFn(row) {
      this.$confirm(this.$t("uexpressPriceCard.offlineInfo"), this.$t('common.mention'), {
        ...
        this.loading = true
        offlineExpireTimeReceiptFuelRateApi(data).then(res => {
          if (res.ok) {
