<template>
  ...
  <el-dialog :close-on-click-modal="false" :title="$t('seaProduct.productLineEdit')" :visible="visible" @close="closeFn"
    ...
    async submitBaseInfo () {
      ...
      this.$refs.formRef.validate(async valid => {
        if (valid) {
          ...
          const res = await editSeaProductLineApi(this.orderId, this.formModel)
          if (res && res.ok) {
            this.$emit('refresh')
