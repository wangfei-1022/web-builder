<template>
	<div class="right-main-wrapper" v-loading="loading">
		<top-search-items-slot :searchFun="list" :resetFun="clearSearchForm" class="mb-10">
			...
			>
         <el-form-item :label="getZH('channel_name')" prop="channelId">
          <ConstantSelect url="/api/trans-service/omc/lnt/channel/list"  v-model="filterForm.channelId" labelKey="channelName" valueKey="id" filterable @change="list" ></ConstantSelect>
        </el-form-item>
			</el-form>
		</top-search-items-slot>
    <div class="mb-10 top-btn-wrap">
			<el-button icon="el-icon-plus" size="mini" type="primary" @click="add()" v-permission="['ovl:channel:priceCard:template:matching:add']">
        {{$t('common.addBtn')}}
			</el-button>
		</div>
		<div >
        ...
        class="ag-theme-balham"
			/>
      <priceCardTemplateAdd :visible.sync="dialog.visible" @saved="list" :type="'11'" />
...
import { getChannelPriceCardBusinessTempApi, delLntPriceCardsTemplateApi } from '@/api/OverseasWarehouse/overseasManage.js'
...
export default {
	...
	components: { priceCardTemOperation, priceCardTemplateAdd },
	computed: {},
	methods: {
		onReady(params) {
		...
		del(row) {
			this.$confirm(this.getZH('delTips'), this.$t('common.tips'), {
				...
				this.loading = true
				delLntPriceCardsTemplateApi(row.id).then(res => {
					if (res.ok) {
		...
		async list() {
			this.loading = true
			const res = await getChannelPriceCardBusinessTempApi(this.filterForm)
			if (res.ok) {
	...
	created() {
		...
		this.columnFields = [
			...
			},
			{
				...
				headerName: this.getZH('correspondingType'),
				minWidth: 120
			},
			{
				field: 'channelName',
				headerName: this.getZH('channel_name'),
				minWidth: 50
