<template>
    ...
    class="mb-10"
		>
          ...
          <el-form-item :label="$t('channelPriceCard.price_card_type')" prop="priceCardType">
						<el-select
							v-model="formData.priceCardType"
							size="mini"
							:placeholder="$t('channelPriceCard.please_select_price_card_type')"
							class="w-full"
							@change="list"
						>
							<el-option :label="$t('channelPriceCard.priceCardType.ALL')" :value="null" />
							<el-option :label="$t('channelPriceCard.priceCardType.GUIDE')" :value="11" />
							<el-option :label="$t('channelPriceCard.priceCardType.LOWER')" :value="12" />
						</el-select>
					</el-form-item>
    ...
    <CopyPriceDialog :visible.sync="copyDialogVisible" :cardType='11' @saved="list" />
...
import { getPriceCardsListApi, publishCostPriceCard, publishTruckCostPriceCard, delPriceCard } from '@/api/OverseasWarehouse/PriceCardManage.js'
...
export default {
	...
	data() {
		return {
			...
			columnDefsRight: [
				...
				{
					headerName: this.$t('channelPriceCard.price_card_type'),
				...
				{
					headerName: this.$t('channelPriceCard.relative_cost_price_card_name'),
					field: 'costPriceCardName'
				},
				{
	...
	methods: {
		...
		async list() {
			...
			const res = await getPriceCardsListApi(this.formData).catch(() => {
				this.loading = false
			})
			if (res.ok) {
	 ...
	 */
		add(id, type, edit, productType) {
			if (id) {
				edit = edit || 0
				this.$router.push({ name: 'ProductCostPriceCardDetail', query: { type, id, edit, productType, time: new Date().getTime() } })
			} else {
