  ...
  <div v-loading="loading" class="right-main-wrapper">
    <section class="info-form">
      <div class="top-box">
        ...
        <el-checkbox v-model="packageForm.checkArea" style="margin-left: 20px;">{{$t('packageWarehouse.checkAreaText')}}</el-checkbox>
      ...
      <div class="table-wrap">
        <div class="total-info-box">
          ...
          <div class="clear-btn-wrap">
            <el-button type="primary" size="mini" style="margin-top: 5px;" @click="printTag" :disabled="singlePrint">{{$t('packageWarehouse.printTag')}}</el-button>
            <el-checkbox style="margin-left: 10px;" v-model="singlePrint">{{$t('packageWarehouse.singlePrint')}}</el-checkbox>
          </div>
        </div>
        <ag-grid-vue
...
import { getBoxInfoApi, getBoxTypeApi, sendOrderNoApi, printOutboundTagApi, recoveryBoxApi } from '@/api/packageWarehouse'
...
export default {
  ...
  methods: {
    ...
    async submitFormData() {
      ...
      */
      ...
      this.resetOrderNo()
      const res = await sendOrderNoApi(_submitObj)
      if (res && res.ok) {
        this.soundObj.weight = ""
        this.soundObj.other = []
        // 出库异常
        if (res.content.failMsgName.length > 0) {
          this.dialogAOPlay(_submitObj, res.content.failMsgName, true)
        ...
        if (res.errors) {
          // 弹窗报错
          this.dialogAOPlay(_submitObj, res.errors[0].message, true)
        } else {
          // 弹窗报ao
          this.dialogAOPlay(_submitObj, this.$t('packageWarehouse.systemError'), false)
        }
      }
      this.loading = false
    },
    ...
    dialogAOPlay(orderObj, errorText, isExt) {
      this.soundObj.weight = ''
      this.soundObj.other = isExt ? ['sound_ao', errorText] : ['sound_ao']
      this.soundTrigger()
      let _msg = `<p>${this.$t('packageWarehouse.codeNo')}: ${orderObj.orderNo} ${this.$t('packageWarehouse.failedToSubmit')}，${this.$t('packageWarehouse.failedReason')}: <span style="color: red;">${errorText}</span></p>`
      const h = this.$createElement
      this.$alert(_msg, '', {
        dangerouslyUseHTMLString: true,
        confirmButtonText: this.$t('common.closed')
      })
    },
    soundTrigger() {
  ...
  mounted() {
    ...
    // const tabArr = this.$store.getters.visitedViews.filter(v => v.name == 'QuicklyInQuicklyOut' || v.name == 'SecondaryMatch' || v.name == 'PackageCheckin' || v.name == 'PackageReceived' || v.name == 'AgentParcelRecived' || v.name == 'BoxPerOrder' || v.name == 'SwapBox')
    // tabArr.forEach(item => {
