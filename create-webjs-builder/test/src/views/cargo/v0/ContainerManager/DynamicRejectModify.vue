...
<script>
  ...
  import {
    getDynamicRejectDetailApi,
    ModifyDynamicRejectDetailApi,
  ...
  export default {
    ...
    methods: {
      async getDetailInfo() {
        this.loading = true
        const res = await getDynamicRejectDetailApi(this.$route.params.id)
        if (res.ok) {
          this.dynamicDetail = res.content
          this.$set(this.upgradeForm,)
          this.operationSelected = [res.content.statusName, res.content.operationCode]
          Object.keys(this.upgradeForm).forEach(key => {this.upgradeForm[key] = res.content[key] || this.upgradeForm[key]})
          // this.upgradeForm = {...this.upgradeForm, ...res.content}
          // this.upgradeForm.containerNo = this.dynamicDetail.containerNo
          // this.upgradeForm.id = this.dynamicDetail.id
      ...
      backToList() {
        ...
        this.$router.push({name: 'DynamicRejectList', query: {refresh: true}})
      },
      async submitForm() {
        this.loading = true
        const res = await ModifyDynamicRejectDetailApi(this.upgradeForm)
        if (res.ok) {
          this.$message({
            type: 'success',
