      ...
      <el-form ref="formData"	:model="formData" label-width="110px" :rules="rules">
        ...
        <el-row :gutter="20">
          <el-col :span="8">
            ...
            <CommonPicker  v-model="formData.pickUpTime" pickerFormat="yyyy-MM-dd"  pikcerType="date" :clearable="false" :disabled="isDisabled" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
          ...
          <!--  -->
          <el-col :span="8">
            <el-form-item :label="getZH('destinationBin')"  prop="warehouseId">
...
import { saveCntPickupApi, detailCntPickupApi, submitCntPickupApi, getCtnPickupApi } from '@/api/OvlSeaSitManage/seaBl'
export default {
	...
	data() {
		...
		return {
			...
			rules: {
				pickUpTime: [{ required: true, message: this.selectZH('deliveryDate') }],
				ctnNo: [{ required: true, message: this.inputZH('ctnNo') }],
				warehouseId: [{ required: true, message: this.selectZH('destinationBin') }],
	...
	methods: {
		...
		ctnNoChange(ctnNo) {
			if (this.$isNotEmpty(ctnNo)) {
				this.loading = true
				getCtnPickupApi({ ctnNo }).then(res => {
					if (res.ok) {
						if (this.$isNotEmpty(res.content)) {
							this.setBlInfo(res.content)
							this.formData.ctnId = res.content.id
							this.formData.warehouseId = res.content.blDetail.warehouseId
							this.formData.warehouseName = res.content.blDetail.warehouseName
							this.formData.returnEmptyDock = res.content.blDetail.pod
						} else {
		...
		async getDetail() {
			this.loading = true
			const res = await detailCntPickupApi(this.id).catch(() => {
				this.loading = false
			})
			if (res.ok) {
		...
		saveForm(refName) {
			this.$refs[refName].validate(valid => {
				...
				saveCntPickupApi(saveForm).then(res => {
					if (res && res.ok) {
		...
		submitData(refName) {
			this.$confirm(this.$t('seaBl.submitTips'), this.$t('common.tips'), {
				...
				this.$refs[refName].validate(valid => {
					...
					const saveForm = this.$extends(true, {}, this.formData)
					submitCntPickupApi(saveForm).then(res => {
						if (res && res.ok) {
							this.$message.success(this.getZH('saveSuccess', 'common'))
							this.backList()
							this.loading = false
	 ...
	 */
		...
		backList() {
			...
			this.$router.push({ name: `ContainerHandleManage` })
		}
	}
}
</script>
