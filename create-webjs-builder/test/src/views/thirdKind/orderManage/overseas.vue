<template>
  <div v-loading="loading" class="pd-16">
    <TopSearchItemsSlot :searchFun="list" :resetFun="reset" class="mb-10">
      <template slot="multipleSearch">
        <el-form ref="formData" :inline="true" :model="formData" class="search-form" label-width="100px">
          ...
          <el-form-item :label="getZH('businessDate')" prop="businessDate">
            <search-date-picker v-model="formData.businessDate" clearable size="mini" />
          </el-form-item>
          <el-form-item :label="getZH('polCode')" prop="polCode">
            <type-select v-model="formData.polCode" type="seaport" @selected="list" />
          </el-form-item>
          <el-form-item :label="getZH('podCode')" prop="podCode">
            <type-select v-model="formData.podCode" type="seaport" @selected="list" />
          </el-form-item>
    ...
    <div class="mb-10 top-btn-wrap">
      ...
      <el-button size="mini" type="primary" @click="importFn" v-permission="['thk:order:import:ovl']">{{getZH('importOrder')}}</el-button>
      <el-button size="mini" type="primary" @click="importOrderChargeFn('AR')" v-permission="['thk:order:import:ovl:charge:ar']">{{getZH('importOrderChargeAR')}}</el-button>
      <el-button size="mini" type="primary" @click="importOrderChargeFn('AP')" v-permission="['thk:order:import:ovl:charge:ap']">{{getZH('importOrderChargeAP')}}</el-button>
      <el-button size="mini" type="primary" @click="exportFn" v-permission="['thk:order:export:ovl:charge']">{{getZH('exportOrder')}}</el-button>
    </div>
    ...
    <el-dialog :title="getZH('addItem')" :visible.sync="dialog.visible" width="400px">
      <div v-if="dialog.visible">
        <el-form ref="addForm" :model="addForm" label-width="95px" label-position="left">
          ...
          <el-form-item :label="getZH('businessDate')" prop="businessDate" :rules="[{ required: true, message: selectZH('businessDate') }]">
    ...
    <ImportOrderChargeDialog ref="importOrderChargeDialogRef" @success="list"/>
...
export default {
  components: { SearchDatePicker },
    ...
    data() {
        return {
            origin: 'ThirdKindOrderListOverseas',
            groupCode: ThirdBusinessGroup.OVERSEAS_WAREHOUSE,
            columnDefsRight: [
                {
