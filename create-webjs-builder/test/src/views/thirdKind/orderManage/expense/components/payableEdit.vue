        ...
        <el-row>
          <el-col :span="6">
            ...
            <el-form-item :label="$t('air.supplier')"
              ...
              :disabled="item.confirmationState === 11" :request-params="{ 'businessType': orderBusinessType }" style="width:  220px;" :defaultDisplay="{id: editForm.scmSupplierId, chineseName: editForm.scmSupplierCnName}" />
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <!-- 结算单位 -->
...
<script>
import { getDictDataApi, getOrderBusinessTaxRateApi } from '@/api/thirdKind'
import { getTaxRateListApi } from '@/api/common'
import { supplierSupplierActiveTitleApi } from "@/api/airExport"
...
export default {
	...
	watch: {
		item: {
			handler(val) {
				...
				this.supplierChange({
					id: target.scmSupplierId,
					chineseName: target.scmSupplierCnName
				})
			},
			immediate: true
		}
	...
	methods: {
		...
		init() {
			...
			Promise.all([getDictDataApi(8), getOrderBusinessTaxRateApi({ businessType: this.orderBusinessType, arAp: 2}), getCurrenciesListApi(), getYHTListApi()]).then(results => {
		...
		async supplierChange(item) {
			...
			if (supplierTitleRes.ok) {
				...
				if (supplierTitleRes.content.length === 1) {
					...
					this.settlementModeOps.push({
						code: supplierTitleMode.rechargeMode,
						name: supplierTitleMode.rechargeModeName
					})
				}
			}
		},
