...
<template>
	<div class="right-main-wrapper" v-loading="loading">
        ...
        <!-- <el-button size="mini" type="primary" @click="agreeFn(false)" >{{$t('fmsPayablePayment.approvalAgree')}}</el-button> -->
        <!-- <el-button size="mini" type="primary" @click="agreeFn(true)" >{{$t('fmsPayablePayment.approvalAgreeLast')}}</el-button> -->
        <!-- <el-button size="mini" type="primary" @click="rejectFn()" >{{$t('fmsPayablePayment.approvalReject')}}</el-button> -->
        <el-button size="mini" type="primary" @click="exportFn()" v-if="role === 'FINANCE'">{{$t('fmsPayablePayment.export')}}</el-button>
...
import NextApprovalUser from "../components/NextApprovalUser";
import {
  getPayablePaymentApplyingApplyApi,
  getPayablePaymentApplyingApplyTotalApi,
  getPayablePaymentApplyingFinanceApi,
  getPayablePaymentApplyingFinanceTotalApi,
  payablePaymentApprovalAgreeApi,
  payablePaymentApprovalRejectApi,
  payablePaymentApprovalExportApi
  ...
  mixins: [mixins],
	...
	data() {
				...
				},
        {
				...
				{
					field: "currentApprovalLog.nextApprovalUser",
					headerName: this.$t('fmsPayablePayment.nextApprovalUser'),
					minWidth: 150,
          cellRendererFramework: "NextApprovalUser"
				},
				{
					field: "operation",
    ...
    exportFn() {
      ...
      params.ids = ids
      let data = {
        instructionPaymentQuery: {
          ...params
        }
      }
      payablePaymentApprovalExportApi(data).then(res => {
        if(res.ok) {
          this.$message.success(this.$t("fmsPayablePayment.export_operate_success")) 
        }
      })
    },
		async getList() {
      let params = this.getParams()
			this.loading = true;
			let fn = this.role === 'FINANCE' ? getPayablePaymentApplyingFinanceApi : getPayablePaymentApplyingApplyApi;
			fn(params).then(res => {
        if (res.ok) {
      ...
      let totalFn = this.role === 'FINANCE' ? getPayablePaymentApplyingFinanceTotalApi : getPayablePaymentApplyingApplyTotalApi
    ...
    rejectFn() {
      let data = {
        ...
        approvalTime: 1702006778870,
        approvalRemark: 'approvalRemark',
        lastNodeFlag: true
      }
      payablePaymentApprovalRejectApi(data).then(res => {
        if (res.ok) {
          this.$message.success(this.$t("fmsPayablePayment.rejectSuccess"))
        }
      })
    },
    agreeFn(isLast) {
      ...
      payablePaymentApprovalAgreeApi(data).then(res => {
        if (res.ok) {
          this.$message.success(this.$t("fmsPayablePayment.approvalSuccess"))
        }
      })
    }
	}
