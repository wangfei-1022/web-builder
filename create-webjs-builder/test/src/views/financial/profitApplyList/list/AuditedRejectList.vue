...
<template>
	<div class="right-main-wrapper" v-loading="loading">
		<top-search-items-slot :searchFun="getList" :resetFun="reset">
            ...
            <el-form-item :label="$t('fmsProfit.businessDate')" prop="businessDatePeriod">
						...
						<el-form-item :label="$t('fmsProfit.submitById')" prop="submitById">
							<base-select v-model="formData.submitById" type="user" @selected="getList"></base-select>
						</el-form-item>
        ...
        <el-button type="primary" @click="batchRecallFn" size="mini">{{$t('fmsProfit.batchRecall')}}</el-button>
			</div>
			<ag-grid-vue
...
export default {
  ...
  mixins: [mixins],
	...
	methods: {
      ...
      // 审核拒绝
      params.profitApplyState = 30
			this.loading = true;
			getProfitApplyListApi(params).then(res => {
        if (res.ok) {
