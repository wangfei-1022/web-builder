<template>
	<div class="right-main-wrapper" v-loading="loading">
        ...
        <el-form-item :label="$t('receiveFms.settlement_mode')" prop="settlementMode">
				...
				<el-form-item :label="$t('receiveFms.bill_date_estimate')" prop="billDateEstimatePeriod" v-if="type === 'AR'">
					<search-date-picker v-model="formData.billDateEstimatePeriod" clearable></search-date-picker>
				</el-form-item>
				<el-form-item :label="$t('receiveFms.bill_date_actual')" prop="billDateActualPeriod">
					<search-date-picker v-model="formData.billDateActualPeriod" clearable></search-date-picker>
				</el-form-item>
				<el-form-item :label="$t('receiveFms.confirm_date_estimate')" prop="confirmDateEstimatePeriod" v-if="type === 'AR'">
					<search-date-picker v-model="formData.confirmDateEstimatePeriod" clearable></search-date-picker>
				</el-form-item>
				<el-form-item :label="$t('receiveFms.confirm_date_actual')" prop="confirmDateActualPeriod">
					<search-date-picker v-model="formData.confirmDateActualPeriod" clearable></search-date-picker>
				</el-form-item>
			</el-form>
		...
		<div class="">
			<div class="top-btn-wrap mb-10 mt-10">
				...
				<el-button size="mini" type="primary" @click="applyInvoice()" v-if="type === 'AR' && !pushEmailState && billState !== 1">{{$t('receiveFms.applyInvoice')}}</el-button>
				<el-button size="mini" type="primary" @click="applyOrHedgeClear(false)" v-if="type === 'AR' && !pushEmailState">{{$t('receiveFms.applyClear')}}</el-button>
				<el-button size="mini" type="primary" @click="applyOrHedgeClear(true)" v-if="type === 'AR' && !pushEmailState">{{$t('receiveFms.hedgeClear')}}</el-button>
				<el-button size="mini" type="primary" @click="sendBillAR()" v-if="type === 'AR' && pushEmailState">
          {{ pushEmailState === 5 ? $t('receiveFms.resendBill') : $t('receiveFms.sendBill')}}
        </el-button>
		...
		<apply-invoice-dialog ref="applyInvoiceDialogRef" origin="BILL" :ids="selectedIds" toPath='FmsApplyForInvoice'></apply-invoice-dialog>
    ...
    <apply-payment-dialog ref="applyPaymentDialogRef" origin="BILL" />
	</div>
</template>
...
import { arBillExportMap } from "../components/constants";
import mixins from './mixins'

export default {
	name: "FmsReceiveBillComponent",
  mixins: [mixins],
	props: {
		invoiceApplyState: {
			type: [String, Number],
	...
	created() {
		let table = this.$extends(true, {}, receiveTable)
    if (this.billState === '' && this.pushEmailState === '') {
      //
    } else if (this.pushEmailState === 5) {
      //
    } else {
      table.schemeContent.splice(21, 2)
    }
    this.columnFields = table;
    this.billExportMap = arBillExportMap;
	}
};
</script>
