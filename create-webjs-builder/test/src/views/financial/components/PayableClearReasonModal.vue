<template>
	<el-dialog :visible.sync="visible" :close-on-click-modal="false" width="400px" :title="$t('fmsPayableClear.clearReason')">
		<div class="container">
			<div v-if="isClearMany" class="des">
				<div class="title"><i class="m-icon el-icon-warning-outline" />
           {{$t('fmsPayableClear.applyClearAmount')}} &gt; {{$t('fmsPayableClear.transactionsAmountUse')}}
        </div>
				<div>{{$t('fmsPayableClear.pleaseSelectReason')}}</div>
			</div>
			<div v-if="!isClearMany" class="des">
				<div class="title"><i class="m-icon el-icon-warning-outline" />
          {{$t('fmsPayableClear.applyClearAmount')}} &lt; {{$t('fmsPayableClear.transactionsAmountUse')}}
	...
	methods: {
		show(isClearMany) {
			...
			if (isClearMany) {
				...
				getDictDataApi(41).then(res => {
					this.reasonList = res.content;
				});
			} else {
				this.isClearMany = false;
				getDictDataApi(42).then(res => {
					if (res.ok) {
						this.reasonList = res.content;
					}
				});
