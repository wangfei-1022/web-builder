<template>
  <div style="height: calc(100vh - 170px)">
    <el-button type="primary" size="mini" style="margin: 5px 0" @click="toShowUploadDialog">上传</el-button>
    <el-table :row-style="mockAgGridRowStyle" :header-row-style="mockAgGridHeaderRowStyle"
      :cell-style="mockAgGridCellStyle" :header-cell-style="mockAgGridHeaderCellStyle" :data="order.fileList"
      style="border: 1px solid #bdc3c7; min-height: 500px" border>
      <!--<el-table-column prop="id" label="id" />-->
      <!--<el-table-column prop="entityId" label="业务实体id" />-->
      <!--<el-table-column prop="entityType" label="业务实体类型" />-->
      <el-table-column prop="fileName" label="文件名" />
      <el-table-column prop="documentTypeCodeDesc" label="文件类型" />
      <el-table-column prop="fileSuffix" label="文件格式" />
      ...
      <el-table-column prop="id" label="操作">
        <template slot-scope="scope">
          <el-button type="text" size="mini" @click="downloadElectronicFile(scope.row)">下载</el-button>
          <el-button type="text" size="mini" @click="toDeleteElectronicFile(scope.row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-dialog :visible.sync="isShowingUploadDialog" width="650px">
        ...
        <div class="right-form">
          <el-form label-width="100px" label-position="left" style="max-width: 400px">
            ...
            <el-form-item label="选择文件">
              <!-- elUploadAction；	:on-success="handleUploadSuccess" :headers="elUploadHeaders"-->
              <!-- <upload-file :show-file-list="false" class="contact-add" @success="handleUploadSuccess"></upload-file> -->
              <!-- <el-upload ref="elUploadElectronicFile" action="" :http-request="httpRequest" :file-list="cloudFileVOList"
                :limit="5" :before-upload="beforeUpload" :on-remove="handleRemove" :on-exceed="
                  () => {
                    this.$message.warning('单次最多上传5个文件');
                  }
                ">
                <el-button size="mini"><i class="el-icon-upload el-icon--right" />上传文件</el-button>
              ...
              <UploadFile @remove="handleRemove" @success="successUpload" :showFileList="true" :multiple="true"
      ...
      <span slot="footer">
        <el-button type="primary" size="mini" @click="toSaveElectronicFile">保存</el-button>
        <el-button size="mini" @click="isShowingUploadDialog = false">取消</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
import { getDocumentTypeCodeTreeApi } from '@/api/airExport'
...
export default {
  ...
  beforeMount () {
    ...
    this.oneUpload.entityId = this.order.id
  },
  methods: {
    /**
* 上传时
*/
    ...
    successUpload (res) {
      ...
      console.log(this.oneUpload.entityType, this.order.id, this.cloudFileVOList)
    },
    // httpRequest (data) {
    ...
    toShowUploadDialog () {
      ...
      if (this.isDocumentTypeCodeLoaded) {
        ...
        getDocumentTypeCodeTreeApi({ parentCode: 'SE' }).then(res => {
    ...
    toSaveElectronicFile () {
      ...
      console.log("this.cloudFileVOList", this.cloudFileVOList)
      saveElectronicFile(this.cloudFileVOList).then(res => {
        ...
        this.order.fileList = res.content.list
      })
    },
