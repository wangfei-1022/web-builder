...
<script>
import { getChargeItemListApi } from "@/api/uexpress/chargeItem";
import { getDictListApi, getChargeCategoryListApi } from "@/api/uexpress/base";
import { AgGridVue } from "ag-grid-vue";

export default {
	name: "UexpressChargeItemList",
	components: {
		AgGridVue
	},
  props: {
      ...
      categoryList: [],
        ...
        feeItemCode: ''
			...
			columnFields: [{
					...
					field: "feeItemLocalName",
					headerName: this.$t("uexpressChargeItem.feeItemLocalName"),
					minWidth: 150
				}, {
					field: "feeItemEnglishName",
					headerName: this.$t("uexpressChargeItem.feeItemEnglishName"),
					minWidth: 150
				}],
	...
	methods: {
		getBaseDataFn() {
			Promise.all([getChargeCategoryListApi()]).then(results => {
				...
				if (categoryRes.ok) {
					this.categoryList = categoryRes.content;
				}
			});
		},
		onReady(params) {
        ...
        arAp: this.arAp,
			...
			const res = await getChargeItemListApi(params);
			if (res.ok) {
