      ...
      <el-form ref="baseForm" :rules="formRule" :model="form" label-width="120px" size="small" class="member-form">
        <div class="member-item">
          ...
          <div class="content">
            ...
            <el-form-item :label="$t('contract.deposit')" prop="deposit.currency" v-if="contractType === 1 || contractType === 3 || contractType === 10">
              <el-row>
                <el-col :span="10">
                  <el-select v-model="form.deposit.currency" size="mini" :placeholder="$t('contract.please_select')" :disabled="contractType === 3" >
                    <el-option v-for="item in currenciesList" :key="item.code" :label="item.code" :value="item.code"/>
                  </el-select>
                </el-col>
                <el-col :span="14">
                  <el-input v-model="form.deposit.amount" :disabled="contractType === 3" oninput="value=value.replace(/[^\d.]/g,'')"></el-input>
                </el-col>
              </el-row>
            </el-form-item>
        ...
        <div class="member-item" v-if="form.contractType === 1 || form.contractType === 2 || form.contractType === 10">
          <div class="title">
            {{$t('contract.business_relative_info')}}
          </div>
          <div class="content">
              <div v-if="form.contractType !== 10" style="margin-buttom: 20px;">
...
<!--                  <account-period :mainId="mainId" :ref="'accountPeriod' + index" :business="item" :contractType="form.contractType"/>-->
                </el-tab-pane>
              </el-tabs>
          </div>
          ...
          <div class="member-item inner" v-if="businessList.length">
            ...
            <div class="content">
              <settlement :mainId="mainId" ref="settlementRef" :activeBusiness="activeBusiness" :contractType="form.contractType" :list="settlementList" :businessList="businessList" :detail="form" :type="type" :role="role"></settlement>
            </div>
          </div>
        ...
        <div class="member-item" v-if="form.contractType === 3  || form.contractType === 20">
          <div class="title">
             {{$t('contract.business_relative_info')}}
          </div>
          <div class="content">
          ...
          <div class="member-item inner">
            ...
            <div class="content" v-if="form.contractType === 10 || form.contractType === 20">
              <settlement :mainId="mainId" ref="settlementRef" :contractType="form.contractType" :list="settlementList" :businessList="businessList" :detail="form"></settlement>
            </div>
            <div class="content" v-if="form.contractType === 3">
               <settlement-form ref="settlementFormRef" :item="customerTitleItem" :step="2"/>
            </div>
          </div>
...
export default {
    ...
    if (this.contractType === 10) { // 如果是票结合同 初始化关联业务
	...
	methods: {
      ...
      if (!res.deposit) {
		...
		getBaseDataFn() {
            ...
            if (this.$route.query.businessType === x.code) {
              this.businessList.push({
                businessType: x.code,
                businessTypeName: x.chineseName
              })
              this.activeBusiness = x.code
            }
					})
				}
				if (salesRes.ok) {
		...
		collectContractData() {
			...
			// 收款取值
			if (this.form.contractType === 1 || this.form.contractType === 2 || this.form.contractType === 10) {
        all.businessTypes = this.businessList
			...
			// 在试单 票结取值  this.form.contractType === 10 ||
			if (this.form.contractType === 3 || this.form.contractType === 20) {
				let business = this.$refs.accountPeriodRef.getValue()
				all.businessTypes = [business]
			}
