<template>
	<div class="right-main-wrapper" v-loading="loading">
		<top-search-items-slot :searchFun="getList" :resetFun="reset">
			<el-form slot="multipleSearch" ref="formData" :inline="true" :model="formData" label-width="100px" size="mini">
				...
				<el-form-item :label="$t('fmsReceiveInvoice.invoice_state_name_invoice')" prop="invoiceState">
		...
		<div class="">
			<div class="top-btn-wrap mb-10 mt-10">
				<!--形票的已申请可以作废 财务-->
				<el-button size="mini" type="primary" @click="nullifyApplyFn" v-if="invoiceType === 3 && applyState === 2 && role ==='FINANCE'">{{$t('fmsReceiveInvoice.cancel_invoice_file')}}</el-button>
        <el-button size="mini" type="primary" @click="recallApplyFn" v-if="invoiceType !== 3 && applyState === 1 && role ==='FINANCE'">{{$t('fmsReceiveInvoice.split_apply')}}</el-button>
			</div>

			<ag-grid-vue-pro :data="dataList" :style-grid="gridHeight" :column-fields-default="columnFields" :grid-ready="onReady"
      :is-no-oper="!((invoiceType === 3 && applyState === 2 && role=='FINANCE') || (invoiceType !== 3 && applyState === 1 && role ==='FINANCE'))" />
    ...
    <recall-tax-apply-modal ref="recallTaxApplyModalRef" @ok="getList" :role="role"/>
	</div>
</template>

<script>
import mixin from "./mixin";
import applyTaxListTableOperator from "./table/applyTaxListTableOperator";
import applyTaxListTableFinance from "./table/applyTaxListTableFinance";
import RecallTaxApplyModal from "./components/RecallTaxApplyModal";
  ...
  components: {
	...
	created() {
		if (this.role === "FINANCE") {
			this.columnFields = applyTaxListTableFinance;
		} else {
			this.columnFields = applyTaxListTableOperator;
		}
	}
};
</script>
