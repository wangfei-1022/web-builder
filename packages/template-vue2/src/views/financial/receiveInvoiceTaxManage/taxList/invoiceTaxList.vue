<template>
	<div class="right-main-wrapper" v-loading="loading">
        ...
        <el-form-item :label='$t("fmsReceiveInvoice.invoiceSeqNo")' prop="invoiceSeqNo">
				...
				<el-form-item :label="$t('fmsReceiveInvoice.invoiceState')" prop="taxInvoiceState">
		...
		<div class="">
			<div class="top-btn-wrap mb-10 mt-10">
				...
				<!--重新提交-异常-->
				<el-button size="mini" type="primary" @click="submitTaxFn()" v-if="invoiceType != 3 && invoiceState == 'EXCEPT'">{{$t("fmsReceiveInvoice.resubmitTax")}}</el-button>
				<!--作废发票-->
				<el-button size="mini" type="primary" @click="nullifyFn()" v-if="invoiceType != 3 && invoiceState == 'INVOICED' && false">{{$t("fmsReceiveInvoice.nullifyInvoice")}}</el-button>
				<!--开具红冲-->
        <el-button size="mini" type="primary" @click="reverseInvocieFn()" v-if="invoiceType != 3 && invoiceState == 'INVOICED'">{{$t("fmsReceiveInvoice.reverseInvocie")}}</el-button>
				<!--回传单号-->
        <el-button size="mini" type="primary" @click="expressNoFn()" v-if="invoiceType != 3 && invoiceState == 'INVOICED'">{{$t("fmsReceiveInvoice.expressNoUpload")}}</el-button>
				<!--下载文件-->
        <el-button size="mini" type="primary" @click="batchDownloadFileFn()" v-if="invoiceType != 3 && invoiceState == 'INVOICED'">{{$t("fmsReceiveInvoice.downloadFile")}}</el-button>
				<!--打印发票-->
        <el-button size="mini" type="primary" @click="printInvoiceFn()" v-if="false && invoiceType != 3 && invoiceState == 'INVOICED'">{{$t("fmsReceiveInvoice.printInvoice")}}</el-button>
				<!--申请核销-->
		...
		<!--作废发票-->
		<el-dialog :title='$t("fmsReceiveInvoice.nullifyInvoice")' :visible.sync="cancelVisible" width="400px" @close="nullifyColseFn">
			<div style="margin-bottom: 10px;">
				<span style="margin-right: 10px;">{{$t("fmsReceiveInvoice.invoiceNo1")}}</span>
				<span>{{cancelInvoiceNo}}</span>
			</div>
			<div>
		...
		<!--开具红冲-->
		<el-dialog :title='$t("fmsReceiveInvoice.reverseInvocie")' v-if="reverseVisible" :visible.sync="reverseVisible" width="400px" @close="reverseInvocieCloseFn">
			<el-form ref="reverseFormRef" :model="reverseForm" :rules="reverseFormRules" label-width="120px" label-position="right">
				<el-form-item :label='$t("fmsReceiveInvoice.taxInvoiceReverseReason")' prop="taxInvoiceReverseReason">
					<el-select v-model="reverseForm.taxInvoiceReverseReason" size="mini">
						<el-option v-for="(item, index) in cancelTypeList" :key="index" :label="item.name" :value="item.code" />
					</el-select>
				</el-form-item>
			</el-form>
			<span slot="footer" class="dialog-footer">
				<el-button @click="reverseInvocieCloseFn" size="mini">{{$t("common.cancel")}}</el-button>
				<el-button type="primary" @click="reverseSubmitFn" size="mini" :loading="cancelLoading">{{$t("common.confirm")}}</el-button>
			</span>
		</el-dialog>
...
import invoiceTaxListFinance from "../table/invoiceTaxListTableFinance";
import { parseTime, formatNumberToThousands, downloadFile } from '@/utils/index'
import { getTaxInvoicedListByFinanceApi, getTaxInvoicedTotalByFinanceApi } from "@/api/financial/fmsReceiveInvoice"
...
export default {
	...
	created() {
		this.columnFields = invoiceTaxListFinance;
	},
  methods: {
    // 税控发票
    async getList() {
      ...
      const res = await getTaxInvoicedListByFinanceApi(params)
      if (res.ok) {
      ...
      getTaxInvoicedTotalByFinanceApi(params).then(res => {
        if (res.ok) {
