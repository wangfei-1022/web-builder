<template>
    ...
    <div class="mb-10 top-btn-wrap">
      ...
      <el-button size="mini" type="primary" @click="storageReservation()"> {{$t('seaBl.storageReservation')}}</el-button>
      ...
      :style="gridHeight"
		...
		<Pagination
			...
			@pagination="list"
		/>
    <ExcelUploadDialog ref="ExcelUploadDialog" tempName="海外仓提柜导入模板.xls" :visible.sync="dialog.visible" @saved="excelUpload" />
	</div>
</template>

<script>
import containerHandleOpBtn from './components/containerHandleOpBtn'
import { getCntPickupListApi, delCntPickupApi, confirmCntPickupApi, backCntPickupApi, importCtnPickupApi, appointInboundApi, checkInboundIsCreatedApi, batchConfirmPickupCtnApi } from '@/api/OvlSeaSitManage/seaBl'
...
export default {
	...
	data() {
		return {
			...
			columnDefsRight: [
				...
				{
					headerName: this.getZH('deliveryDate'),
					field: 'pickUpTime',
					minWidth: 140,
				...
				{
					headerName: this.getZH('destinationBin'),
					field: 'warehouseName',
					minWidth: 120
				},
				{
				...
				{
					headerName: this.getZH('appointInboundNo'),
	...
	beforeRouteEnter(to, from, next) {
		next(vm => {
			if (from.name === 'ContainerHandleDetail') {
				vm.list()
			}
		})
	},
	...
	methods: {
		...
		storageReservation() {
			...
			// 过滤没有确认的数据
			selectedList = selectedList.filter(item => item.state === 2)
			if (this.$isEmpty(selectedList)) {
				this.$message.error(this.getZH('tips1'))
				return
			}
			let ids = []
			...
			checkInboundIsCreatedApi({ ids }).then(rs => {
				if (rs.ok) {
					...
					if (this.$isNotEmpty(rs.content.uncreatedList)) {
						msg += this.getZH('ctnNo')
						rs.content.uncreatedList.forEach(item => {
							saveIds.push(item.id)
							msg += `【${item.ctnNo}】`
							msg += this.getZH('tips3')
						})
					} else {
						msg += this.getZH('tips4')
					}
		...
		batchComfirm() {
			...
			this.confirmTips(`${this.getZH('tips6')}${selectedList.length}${this.getZH('tips7')}`).then(() => {
				this.loading = true
				batchConfirmPickupCtnApi({ ids }).then(res => {
					if (res.ok) {
						this.loading = false
		...
		async list() {
			this.loading = true
			const res = await getCntPickupListApi(this.formData).catch(() => {
				this.loading = false
			})
			if (res && res.ok) {
		...
		confirmData(id) {
			this.confirmTips(this.$t('seaBl.confirmTips')).then(() => {
				this.loading = true
				confirmCntPickupApi(id).then(res => {
					if (res.ok) {
		...
		// 导入
		excelUpload(data) {
			importCtnPickupApi(data).then(res => {
				if (res.ok) {
		 ...
		 */
		edit(operate, id) {
			this.$router.push({ name: `ContainerHandleDetail`, params: { operate, id } })
		},
		/**
		 * 重置搜索条件
		 */
