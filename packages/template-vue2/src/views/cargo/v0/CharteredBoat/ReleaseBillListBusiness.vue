<template>
  <div class="sys-right-inner-wrap" v-loading="loading">
      ...
      <template #mutliple>
        <div class="multiple-form-wrap">
          <el-form ref="multipleForm" :inline="true" :model="multipleForm" label-width="80px">
            ...
            <el-form-item :label="`${$t('releaseBillList.confirmTime')}: `" prop="status">
              <el-date-picker
    ...
    <el-dialog v-loading="loading" :title="$t('releaseBillList.releaseConfirmText')" top="5vh" :visible.sync="dialogVisible" :close-on-click-modal="false" :before-close="closeConfirmDialog" class="release-confirmation" width="85%">
      <div style="width: 100%; min-width: 800px; overflow: auto; padding-bottom: 20px">
        <el-form :inline="false" label-width="100px" :model="confirmationGuaranteeForm" @submit.native.prevent ref="confirmationGuaranteeForm" style="width: 100%;min-width: 1200px;max-height: 720px; overflow: auto">
          <div style="display: flex;">
            <div style="width: 35%;max-height: 500px; min-height: 300px;padding-right: 10px; ">
              ...
              <el-form-item :label="$t('releaseBillList.declareBlNo')" prop="declareStatus">
                <el-collapse v-for="(item,index) in confirmationGuaranteeForm.blNos" :key="index"  v-model="confirmationGuaranteeForm.selBlNos" @change="selBlNos" accordion style=" padding-top: 6px">
            ...
            <div style="width: 65%">
              <div style="display: flex">
                <el-form-item :label="$t('releaseBillList.status')" prop="status" label-width="71px" :rules="[{ required: true, message: $t('releaseBillList.statusEmptyInvalid'), trigger: 'change' },]">
                  <el-radio-group v-model="confirmationGuaranteeForm.status" @change="passStatusChange">
                    <el-radio :label="2">{{$t('releaseBillList.reject')}}</el-radio>
                    <el-radio :label="3">{{$t('releaseBillList.pass')}}</el-radio>
                  </el-radio-group>
                </el-form-item>
                <div style="margin: 5px 0 0 50px">
                  <el-button @click="paymentNotReceived(confirmationGuaranteeForm.id)" type="primary" size="mini" icon="el-icon-s-release">{{$t('releaseBillList.paymentNotReceived')}}</el-button>
                </div>
              </div>
              <el-form-item v-if="confirmationGuaranteeForm.status === 2" :label="$t('releaseBillList.reasonOfRejected')" label-width="71px" prop="rejectReason" :rules="[{ required: true, message: $t('releaseBillList.reasonEmptyInvalid'), trigger: 'blur' },]">
              ...
              <el-form-item v-if="confirmationGuaranteeForm.status === 3" :label="$t('releaseBillList.remarkVerify')" label-width="71px" prop="businessPassRemark">
                <el-input v-model="confirmationGuaranteeForm.businessPassRemark" type="textarea" rows="4" maxlength="500" style="width: 600px;" show-word-limit :placeholder="$t('releaseBillList.remarkVerifyPlaceholder')" ></el-input>
              </el-form-item>

              <div class="table-wrap mb-10">
                <el-dropdown size="mini" @command="picturePreview">
                  <el-button type="primary" size="mini">
                    {{$t('releaseBillList.releaseFileView')}}<i class="el-icon-arrow-down el-icon--right"></i>
                  </el-button>
                  <el-dropdown-menu slot="dropdown">
                    <el-dropdown-item v-for="(item,index) in confirmationGuaranteeForm.files" v-if="item.fieldCode === 'release'" :command="index" :key="index">{{ item.originFileName }}</el-dropdown-item>
                  </el-dropdown-menu>
                </el-dropdown>
                <el-dropdown class="ml-10" size="mini" @command="picturePreview">
                  ...
                  <el-dropdown-menu slot="dropdown">
                    <el-dropdown-item v-for="(item,index) in confirmationGuaranteeForm.files" v-if="item.fieldCode === 'payment'" :command="index" :key="index">{{ item.originFileName }}</el-dropdown-item>
                  </el-dropdown-menu>
                </el-dropdown>
...
import {
  ...
  releaseListBusinessApi, updateAccountStatusApi,
  updateStatusByBusinessApi
...
import {getBLList, issueStatisticApi} from "@/api/bl"
export default {
  name: 'ReleaseBillListBusiness',
  components: { cargoAdvanceSelect, topSearchItems, RemoteSearchSelector,
    'incomeComponent': {
      template: `<el-button v-if="params.data.status === 1" type="text" size="mini" @click="params.context.componentParent.confirm(params.data.id)">{{params.btnView}}</el-button>`,
      methods: {}
    },
    'viewPictureRelease': {
  ...
  methods: {
    ...
    async getList() {
      this.loading = true
      const res = await releaseListBusinessApi(this.multipleForm)
      this.loading = false
      if (res.ok) {
    ...
    clearSearchForm() {
      ...
      this.multipleForm.releaseNo = null
    ...
    async confirm(id) {
      if (id) {
        ...
        if (res.ok) {
          ...
          let index = res.content.files.findIndex(item => item.fieldCode === 'payment')
    ...
    passStatusChange() {
      ...
      this.$set(this.confirmationGuaranteeForm, 'businessPassRemark', null)
      this.$refs.confirmationGuaranteeForm.clearValidate(['rejectReason', 'businessPassRemark'])
    },
    sendConfirms() { // 审核
      this.$refs.confirmationGuaranteeForm.validate(async valid => {
        if (valid) {
          ...
          let data = {
            ...
            businessPassRemark: this.confirmationGuaranteeForm.businessPassRemark
          }
          const res = await updateStatusByBusinessApi(data, this.confirmationGuaranteeForm.id)
          this.loading = false
          if (res.ok) {
    ...
    async paymentNotReceived(id) { // 款项未到账
      this.loading = true
      const res = await updateAccountStatusApi({accountStatus: false}, id)
      this.loading = false
      if (res.ok) {
        this.closeConfirmDialog()
        this.getList()
  ...
  async created() {
    if (this.$route.query.releaseNo) {
      this.$set(this.multipleForm, 'releaseNo', this.$route.query.releaseNo)
    }
