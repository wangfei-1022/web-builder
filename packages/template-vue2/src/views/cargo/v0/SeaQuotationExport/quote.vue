<template>
  <div class="app" v-loading="loading">
    <div>
      ...
      <div class="operation-btn" v-show="!quoteVisible">
        <el-button type="primary" @click="quoteVisible = true">{{ $t('seaQuotationExport.quote') }}</el-button>
        <el-button type="danger" @click="unableQuote">{{ $t('seaQuotationExport.unableQuote') }}</el-button>
        <el-button @click="backList">{{ $t('seaQuotationExport.cancel') }}</el-button>
      </div>
      <div v-show="quoteVisible">

        <div style="margin-bottom: 20px;">
          <el-button size="mini" type="primary" @click="addTab(editableTabsValue)" style="margin-left:10px">
            {{ $t('seaQuotationExport.addOrder') }}
          </el-button>
        </div>
        <el-tabs v-model="editableTabsValue" type="card" closable @tab-remove="removeTab" @tab-click="clickTab">
          <el-tab-pane v-for="(item) in editableTabs" :key="item.name" :label="item.title" :name="item.name">
            <el-form ref="orderForm" :model="item.order" :rules="orderRules" label-width="95px" label-position="left"
              ...
              <div class="form-module-item">
                ...
                <el-form-item :label="$t('seaQuotationExport.shippingCompany')"
                  ...
                  prop="carrierName">
                  <cargo-limited-select placeholder="" :select-label="item.order.carrierName" size="mini"
                    :select-value="item.order.carrierName" type="carrier" style="width: 100%;" @set="selectedCarrier"
                    clearable />
                </el-form-item>
                ...
                <el-form-item :label="$t('seaQuotationExport.sailTime')" prop="etd">
                  <el-row>
                    <el-col :span="19">
                      <CommonPicker v-model="item.order.etd" pikcer-type="datetime" pickerFormat="yyyy-MM-dd HH:mm"
                        size="mini" @change="changeByEtd(item.order)" />
                    </el-col>
                ...
                <el-form-item :label="$t('seaQuotationExport.trans')" prop="transCode"
                  ...
                  <el-row>
                    <el-col :span="10">
                      <type-select :disabled="Number(item.order.transType) === 10" v-model="item.order.transCode"
                        type="seaport" @selected="selectedTrans" size="mini" />
                    </el-col>
                ...
                <el-form-item :label="$t('seaQuotationExport.cutOffTime')" prop="cutOffTime">
                  ...
                  <el-row>
                    <el-col :span="19">
                      <el-date-picker v-model="item.order.cutOffTime" type="datetime"
                        :picker-options="disabledDate(item.order)" clearable size="mini" style="width:100%"
                        format="yyyy-MM-dd HH:mm" @change="changeByTime(item.order)" />
                    </el-col>
                    <span>{{ weekItem[item.order.cutOffTimeWeek - 1] }}</span>
                  </el-row>
                  <!-- <CommonPicker v-model="item.order.cutOffTime" pikcer-type="datetime" pickerFormat="yyyy-MM-dd HH:mm"
                    :picker-options="disabledDate(item.order)" size="mini" /> -->
                </el-form-item>


                <!-- 报价有效期 -->
                <el-form-item :label="$t('seaQuotationExport.quoteValidityTime')" prop="validTimeRange">
                  <el-date-picker v-model="item.order.validTimeRange" value-format="timestamp" type="daterange"
                ...
                <el-form-item :label="$t('seaQuotationExport.capacity')" prop="capacity">
                  <el-input v-model="item.order.capacity" size="mini" />
                </el-form-item>
                <!-- 注意事项 -->
            ...
            <el-form ref="chargeForm" class="order-air-item" :model="item.order.chargeForm" label-position="left">
              <el-table size="mini" :border="true" :data="item.order.chargeForm.charges" style="width: 100%; "
                row-key="id">
                <el-table-column prop="feeName" :label="$t('receiveFms.charge_name')">
                  <template slot-scope="{row, $index}">
                    <el-form-item :rules="[{ required: true, message: $t('receiveFms.can_not_be_null') }]"
                      ...
                      <type-select v-model="row.feeName" type="businessFee" :disabled="row.feeCode === 'OFRT'"
                ...
                <el-table-column prop="unitName1" :label="$t('receiveFms.unit_name')">
                  <template slot-scope="{row, $index}">
                    <el-form-item :rules="[{ required: true, message: $t('receiveFms.can_not_be_null') }]"
                      ...
                      <el-select v-model="row.unit" size="mini" :disabled="row.feeCode === 'OFRT'"
                        @change="unitlChange($event, $index)">
                ...
                <el-table-column prop="currency" :label="$t('receiveFms.currency')">
                  <template slot-scope="{row, $index}">
                    <el-form-item :rules="[{ required: true, message: $t('receiveFms.can_not_be_null') }]"
                      ...
                      <el-select v-model="row.currency" size="mini" :disabled="row.feeCode === 'OFRT'">
                  ...
                  <template slot-scope="{row, $index}">
                    <el-form-item :rules="[{ required: true, message: $t('receiveFms.can_not_be_null') }]"
                      ...
                      <el-input v-model="row.price" size="mini" v-input-limit="[-9, 2]"
                        :disabled="row.feeCode === 'OFRT' || row.unit !== 2004"></el-input>
                    </el-form-item>
                  </template>
                </el-table-column>
                <el-table-column prop="twentyGP" label="20GP">
                  <template slot-scope="{row, $index}">
                    <el-form-item :rules="[{ required: true, message: $t('receiveFms.can_not_be_null') }]"
                      ...
                      <el-input v-model="row.twentyGP" size="mini" v-input-limit="[-9, 2]"
                        :disabled="editStatus(row, '20GP')"></el-input>
                    </el-form-item>
                  </template>
                </el-table-column>
                <el-table-column prop="fortyGP" label="40GP">
                  <template slot-scope="{row, $index}">
                    <el-form-item :rules="[{ required: true, message: $t('receiveFms.can_not_be_null') }]"
                      ...
                      <el-input v-model="row.fortyGP" size="mini" v-input-limit="[-9, 2]"
                        :disabled="editStatus(row, '40GP')"></el-input>
                    </el-form-item>
                  </template>
                </el-table-column>
                <el-table-column prop="fortyHQ" label="40HQ">
                  <template slot-scope="{row, $index}">
                    <el-form-item :rules="[{ required: true, message: $t('receiveFms.can_not_be_null') }]"
                      ...
                      <el-input v-model="row.fortyHQ" size="mini" v-input-limit="[-9, 2]"
                        :disabled="editStatus(row, '40HQ')"></el-input>
                    </el-form-item>
                  </template>
                </el-table-column>
                <el-table-column prop="fortyFiveHQ" label="45HQ">
                  <template slot-scope="{row, $index}">
                    <el-form-item :rules="[{ required: true, message: $t('receiveFms.can_not_be_null') }]"
                      ...
                      <el-input v-model="row.fortyFiveHQ" size="mini" v-input-limit="[-9, 2]"
                        :disabled="editStatus(row, '45HQ')"></el-input>
                    </el-form-item>
                  </template>
                </el-table-column>
                <el-table-column prop="fortyNOR" label="40NOR">
                  <template slot-scope="{row, $index}">
                    <el-form-item :rules="[{ required: true, message: $t('receiveFms.can_not_be_null') }]"
                      ...
                      <el-input v-model="row.fortyNOR" size="mini" v-input-limit="[-9, 2]"
                        :disabled="editStatus(row, '40NOR')"></el-input>
                    </el-form-item>
                  </template>
                </el-table-column>
                ...
                <el-table-column prop="oper" :label="$t('receiveFms.operation')" width="60">
                  <template slot-scope="{$index,row}">
                    <i class="iconfont icon-minus-circle"
                      v-if="item.order.chargeForm.charges.length > 1 && row.feeCode !== 'OFRT'"
        ...
        <div class="operation-btn">
          <el-button type="primary" @click="addOrder">{{ $t('seaQuotationExport.save') }}</el-button>
          <el-button @click="backList">{{ $t('seaQuotationExport.cancel') }}</el-button>
        </div>
      </div>
    </div>
...
export default {
  ...
  data () {
    ...
    return {
      ...
      setDateRange: this.disabledDate(),
      fileimageShow: false,
      filepaths: [],
      unitList: [],
      currencyList: [],
  ...
  created () {
    ...
    getSeaQuotationExportDetailApi(this.$route.query.id).then(res => {
      ...
      try {
        ...
        // 柜型柜量
        let ctnInfos = ""
        // let ctnInfoList = res.content.ctnInfo ? JSON.parse(res.content.ctnInfo) : []
        res.content.ctnInfoList.forEach(info => {
          ctnInfos += info.ctnTypeDesc + "*" + info.ctnCount + ","
          // ctnInfos += "45HQ*" + info.ctnCount + ","
        })
  ...
  methods: {
    changeByEtd (item) {
      item.cutOffTime = ''
      this.disabledDate(item)
      this.changeByTime(item)
    },
    changeByTime (item) {
      item.etdWeek = this.weekList[new Date(item.etd).getDay()]
      item.cutOffTimeWeek = this.weekList[new Date(item.cutOffTime).getDay()]
    },
    ...
    checkRequired () {
      ...
      this.editableTabs.forEach(function (v) {
        ...
        v.order.chargeForm.charges.forEach(item => {
          ...
          if (item.feeCode !== "OFRT") {
            if (Number(item.unit) === 2004) {
              if (!item.price) {
                flag = true
              }
            }
    ...
    addOrder () {
      ...
      this.editableTabs.forEach(element => {
        ...
        element.order.cutOffTimeWeek = this.weekList[new Date(element.order.cutOffTime).getDay()]
        element.order.etdWeek = this.weekList[new Date(element.order.etd).getDay()]
        element.order.cutOffTime = new Date(element.order.cutOffTime).valueOf()
        element.order.offerType = 10 // 10人工报价

        element.order.chargeForm.charges.forEach(chage => {
          let chargeInfoList = []
          if (chage.price) {
            let chargeInfo = { "price": chage.price }
            chargeInfoList.push(chargeInfo)
          }
          if (chage.twentyGP) {
            let chargeInfo = { "ctnType": 1, "price": chage.twentyGP }
            chargeInfoList.push(chargeInfo)
          }
          if (chage.fortyGP) {
            let chargeInfo = { "ctnType": 2, "price": chage.fortyGP }
            chargeInfoList.push(chargeInfo)
          }
          if (chage.fortyHQ) {
            let chargeInfo = { "ctnType": 3, "price": chage.fortyHQ }
            chargeInfoList.push(chargeInfo)
          }
          if (chage.fortyFiveHQ) {
            let chargeInfo = { "ctnType": 4, "price": chage.fortyFiveHQ }
            chargeInfoList.push(chargeInfo)
          }
          if (chage.fortyNOR) {
            let chargeInfo = { "ctnType": 7, "price": chage.fortyNOR }
            chargeInfoList.push(chargeInfo)
          }

          chage.chargeInfo = JSON.stringify(chargeInfoList)
        })
