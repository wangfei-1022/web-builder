<template>
  <div class="box" v-loading="loading">
    <el-form :inline="true" :model="packingDataList" @submit.native.prevent ref="customerInformationForm">
      ...
      <div class="box_child">
          <div class="flex-bet" style="max-width: 1200px; flex-wrap:nowrap">
...
<!--                  <el-radio :label="2" style="padding-left: 10px">HBL</el-radio>-->
              ...
              <el-form-item :label="$t('sea.issuingMethod')" prop="releaseModeCode" size="mini"  :rules="[ { required: true,message:$t('sea.placeholder'),trigger: 'change'} ]" class="issuing_method">
                <el-radio-group v-model="packingDataList.releaseModeCode" @change="releaseModeCodeChange">
                  ...
                  <el-radio label="SWB" v-if="packingDataList.blSignType === 1">SWB</el-radio>
                  <el-radio label="TELEX RELEASE">TELEX RELEASE</el-radio>
                </el-radio-group>
              </el-form-item>
            </div>
            <el-form-item :label="$t('sea.SO')" prop="bookingNo" size="mini" style="display: flex">
              <div style="width: 300px">
                <el-button type="primary" plain size="mini" v-for="item in packingDataList.bookingNos" :key="item" @click="bookingNosClick(item)"  class="booking_nos">{{ item }}</el-button>
              </div>
            </el-form-item>
            <el-form-item :label="$t('sea.billNo')" prop="blNo" size="mini" :rules="[ { required: true,message:$t('sea.placeholder'),trigger: 'change'} ]">
              <el-input type="text" placeholder="" v-trim v-model="packingDataList.blNo" v-input-auto="'upperCase'" ></el-input>
            </el-form-item>
        </div>
      </div>
<!--      R24 反恐申报信息-->
      <R24 v-if="packingDataList.blSignType === 1"  name="carrier" :packingDataList="packingDataList" @declareBlFlagChange="declareBlFlagChange" @seaBlJobR24SaveDTOListChange="seaBlJobR24SaveDTOListChange" ref="R24"></R24>
      <!--      装箱信息-->
...
<!--          v-if="[30,40].indexOf(customerInformationForm.orderState)>-1"-->
          <el-button type="primary" size="mini" @click="getPackingInformation()">{{$t('sea.getPackingInformation')}}</el-button>
...
<!--      v-if="customerInformationForm.orderState>=30 && customerInformationForm.orderState!==100"-->
      <div class="box_child" >
        ...
        <el-table :data="packingDataList.cargoList" header-row-class-name="el-grid-header el-grid-header-required " class="cargoList" style="width: 100%; margin: 10px 0;">
          ...
          <el-table-column prop="hscode" label="HS CODE" class-name="not-required" width="120"></el-table-column>
          <el-table-column prop="quantity" :label="$t('sea.number')" class-name="not-required" width="120"></el-table-column>

          <el-table-column prop="packageType" :label="$t('sea.packageType')" class-name="not-required" width="120"></el-table-column>
          <el-table-column prop="grossWeight" :label="$t('sea.grossWeight')" class-name="not-required" width="120"></el-table-column>
          <el-table-column prop="volume" :label="$t('sea.volume')" class-name="not-required" width="120"></el-table-column>
          <el-table-column prop="cargoDesc" :label="$t('sea.marks')" class-name="not-required" width="120"></el-table-column>
          <el-table-column v-if="declareBlFlag && packingDataList.blSignType === 1" prop="nvoccHblNo" :label="$t('sea.nvoccFreightForwarderBillOfLadingNumber')">
            <template slot-scope="{row,$index}">
              <el-form-item label="" :prop="'cargoList.' + $index + '.nvoccHblNo'" size="mini" :rules="[ { required: true,message:$t('sea.placeholder'),trigger: 'change'} ]">
                <el-select v-model="row.nvoccHblNo" size="mini">
                  <el-option v-for="item in nvoccHblNoList" :label="item" :value="item" :key="item"></el-option>
                </el-select>
              </el-form-item>
            </template>
          </el-table-column>
          ...
          <div style="width: 49%">
            ...
            <div class="shippingFee" >
              ...
              <el-form-item  :label="$t('sea.whereToPay')" prop="oceanPlace" >
                <el-input type="text" v-trim v-input-auto="'EN_num_symbol'" v-model="packingDataList.oceanPlace" style="width: 467px" size="mini" :placeholder="$t('sea.pleaseEnter')"></el-input>
              </el-form-item>
              <div style="display: flex">
          ...
          <div style="width: 49%">
            ...
            <div class="shippingFee voucherPreparation">
              <div style="display: flex">
                ...
                <el-button  type="text" icon="el-icon-circle-plus" class="icon blue userReferenceBtn"  @click="addProcessing"></el-button>
              </div>
              <div v-for="(item,i) in packingDataList.seaBlJobDocumentationRequestDTOList" :key="i" style="display: flex">
                ...
                <el-button  type="text" icon="el-icon-remove" class="icon red userReferenceBtn"  @click="delProcessing(i)"></el-button>
              </div>
            </div>
...
<!--        付款指示-->
        <div class="content_box payment_instructions" style="display: block">
          ...
          <div v-for="(item,i) in packingDataList.seaBlJobSurchargeSaveDTOList" :key="i" class="seaBlJobSurchargeSaveDTOList" style="display: flex" >
            ...
            <el-button  type="text" icon="el-icon-remove" class="icon red userReferenceBtn"  @click="delPaymentInstructions(i)" style="margin-top: -6px;"></el-button>
          </div>
        </div>
    ...
    <div class="btn_box" :style="{width: `calc(100% - ${withoutAnimation ? 180 :54}px)`}">
      ...
      <el-button v-if="!blId || [-1].indexOf(packingDataList.jobState)>-1" type="primary" @click="replenishmentCompleted" size="mini">{{ $t('sea.replenishmentCompleted') }}</el-button>
    ...
    <el-dialog :title="Number(editableTabsValue) === 4 ? $t('sea.getPackingInformation'): $t('sea.adjustCustomerBill')" :visible.sync="dialogShow" :close-on-click-modal="false" :before-close="closeDialog" width="1100px" class="dialog-box">
<!--      <el-form :inline="true" :model="dialogForm" @submit.native.prevent ref="dialogForm" style="display: flex" class="dialogForm">-->

<!--      </el-form>-->
      <el-tree :data="treeData" :props="defaultProps" node-key="value" @node-click="handleNodeClick" ref="tree" show-checkbox></el-tree>
...
export default {
  ...
  created() {
    ...
    console.log('this.customerBlForm,',this.packingDataList)
    // if(!this.blId && !this.packingDataList.rateType){
      if(this.packingDataList.freightTermCode === 'PREPAID'){ // 预付
        this.$set(this.packingDataList, 'rateType', 'Service Contract Number')
      }else{
        this.$set(this.packingDataList, 'rateType', 'CSO/Agreement Number')
      }
    // }
    // 贸易条款
  ...
  methods: {
    ...
    calculate(val,change = true) {
      ...
      val.map(item => {
        ...
        bookingNos.push(item.soNo)
      })
      this.$set(this.packingDataList, 'bookingNos', lodash.uniq(bookingNos))
      if(this.packingDataList.bookingNos.length === 1){
        if(change) {
          this.$set(this.packingDataList, 'blNo', this.packingDataList.bookingNos[0])
        }
      }else if(this.packingDataList.bookingNos.length > 1){
        ...
        if(change){
          this.$set(this.packingDataList,'blNo',this.packingDataList.bookingNos[minArr])
        }
      }
      if (this.$isEmpty(val)) {
    ...
    bookingNosClick(val){
      this.$set(this.packingDataList,'blNo',val)
    },
    ...
    billCopySun(){
      let sum = Number(this.packingDataList.bookingPartyCopy) +Number(this.packingDataList.shipperCopy) +Number(this.packingDataList.freightForwarderCopy) +
        Number(this.packingDataList.consigneeCopy) +Number(this.packingDataList.notifyCopy)
      if(sum>9){
    ...
    async next() {
      this.$refs.customerInformationForm.validate(async valid => {
        if (valid) {
          ...
          if(this.packingDataList.blSignType === 1){
            let seaBlJobR24SaveDTOList = this.$refs.R24.dataForm.seaBlJobR24SaveDTOList
            this.packingDataList.seaBlJobR24SaveDTOList = seaBlJobR24SaveDTOList
          }
          let ids = this.packingDataList.cargoList.map(item => item.id)
...
.btn_box{ position: fixed; bottom: 0px; right: 0; text-align: right; background: #fff; padding: 15px 50px 15px 0; z-index: 9}
...
.booking_nos{ margin: 0 0 5px 10px}
